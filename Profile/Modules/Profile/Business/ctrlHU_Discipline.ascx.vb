    ...
    Protected Sub InitControl()
        ...
        Try
            ...
            Common.Common.BuildToolbar(Me.MainToolBar, ToolbarItem.Create,
                                       ...
                                       ToolbarItem.Approve, ToolbarItem.Unlock)
            CType(MainToolBar.Items(6), RadToolBarButton).Text = "Mở phê duyệt"
        ...
        Try
            ...
            Select Case CurrentState
                ...
                Case CommonMessage.STATE_APPROVE
                    ...
                    Next
                    If rep.ApproveListDiscipline(lst, "PD") Then
                        Refresh("UpdateView")
                        UpdateControlState()
                    Else
                ...
                Case CommonMessage.ACTION_UNLOCKED
                    ...
                    For Each item As GridDataItem In rgDiscipline.SelectedItems
                        lst.Add(item.GetDataKeyValue("ID"))
                    Next
                    If rep.ApproveListDiscipline(lst, "CPD") Then
                        Refresh("UpdateView")
                        UpdateControlState()
                    Else
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_FAIL), NotifyType.Error)
...
#Region "Event"
        ...
        Try
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_DELETE
                    ...
                    For Each item As GridDataItem In rgDiscipline.SelectedItems
                        If item.GetDataKeyValue("STATUS_ID") IsNot Nothing AndAlso item.GetDataKeyValue("STATUS_ID") = ProfileCommon.DISCIPLINE_STATUS.APPROVE_ID Then
                            ShowMessage(Translate("Kỷ luật đã phê duyệt. Không thể xóa."), NotifyType.Warning)
                            Exit Sub
                        End If
                    Next
                ...
                Case CommonMessage.TOOLBARITEM_PRINT
                    ...
                    listID.Add(rgDiscipline.SelectedValue)
                    If repval.ValidateBusiness("HU_DISCIPLINE", "ID", listID) Then
                    ...
                    Dim item_1 = rgDiscipline.SelectedItems
                    ...
                    For Each lst As GridDataItem In item_1
                        ...
                        lstEmpId = lstEmpId + "," + lst.GetDataKeyValue("EMPLOYEE_ID").ToString
                    Next
                    lstIDs = lstIDs.Remove(0, 1)
                    lstEmpId = lstEmpId.Remove(0, 1)
                    ...
                    Using rep As New ProfileRepository
                        ...
                        '                               folderName)
                        dtData = rep.GetHU_DataDynamic_Muti_1(lstIDs, lstEmpId,
                    ...
                    If Not Utilities.GetTemplateLinkFile("KL",
                                                         folderName,
                                                         filePath,
                                                         extension,
                                                         iError) Then
                ...
                Case CommonMessage.TOOLBARITEM_UNLOCK
                    ...
                    For Each item As GridDataItem In rgDiscipline.SelectedItems
                        If item.GetDataKeyValue("STATUS_ID") IsNot Nothing AndAlso item.GetDataKeyValue("STATUS_ID") = ProfileCommon.DISCIPLINE_STATUS.WAIT_APPROVE_ID Then
                            ShowMessage(Translate("Kỷ luật đang chờ phê duyệt. Vui lòng kiểm tra lại."), NotifyType.Warning)
                            Exit Sub
                        End If
                    Next

                    ctrlMessageBox.MessageText = "Bạn có chắc chắn muốn mở phê duyệt ?"
    ...
    Private Sub ctrlMessageBox_ButtonCommand(ByVal sender As Object, ByVal e As MessageBoxEventArgs) Handles ctrlMessageBox.ButtonCommand
        ...
        Try
            ...
            If e.ActionName = CommonMessage.TOOLBARITEM_UNLOCK And e.ButtonID = MessageBoxButtonType.ButtonYes Then
                CurrentState = CommonMessage.ACTION_UNLOCKED
                UpdateControlState()
            End If
            _mylog.WriteLog(_mylog._info, _classPath, method, CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub rgDiscipline_ItemDataBound(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rgDiscipline.ItemDataBound
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Try
            Dim startTime As DateTime = DateTime.UtcNow
...
#Region "Custom"
            ...
            If lstID.Count > 0 Then
                ...
                For Each item As GridDataItem In rgDiscipline.SelectedItems
                    If item.GetDataKeyValue("STATUS_ID") = ProfileCommon.DECISION_STATUS.APPROVE_ID Then
                        ShowMessage(Translate("Các kỷ luật được chọn đã phê duyệt"), NotifyType.Warning)
                        Exit Sub
                    End If
                Next
                ...
                End If
            Else
                ShowMessage("Các kỷ luật được chọn đã phê duyệt", NotifyType.Information)
            End If
            rep.Dispose()
            _mylog.WriteLog(_mylog._info, _classPath, method, CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
    ...
    Private Sub Open_ApproveDiscipline()
        ...
        Try
            ...
            If lstID.Count > 0 Then
                ...
                End If
            Else
                ShowMessage("Các kỷ luật được chọn đang ở trạng thái chờ phê duyệt", NotifyType.Warning)
            End If
            rep.Dispose()
            _mylog.WriteLog(_mylog._info, _classPath, method, CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Try
            ...
            'If chkOtherEmp.Checked Then
            '    _filter.EMPLOYEE_ID = -1
            'End If
    ...
    Private Sub rgDiscipline_PreRender(sender As Object, e As EventArgs) Handles rgDiscipline.PreRender
        Try
            If CommonConfig.APP_SETTING_12() Then
                rgDiscipline.MasterTableView.GetColumn("SIGNER_NAME").Visible = False
                rgDiscipline.MasterTableView.GetColumn("SIGNER_TITLE").Visible = False
                rgDiscipline.MasterTableView.GetColumn("SIGN_DATE").Visible = False
            End If
        Catch ex As Exception
            Throw ex
        End Try
