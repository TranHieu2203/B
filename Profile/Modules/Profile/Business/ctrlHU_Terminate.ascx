<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ctrlHU_Terminate.ascx.vb"
    Inherits="Profile.ctrlHU_Terminate" %>
        ...
        <tlk:RadSplitter ID="RadSplitter3" runat="server" Width="100%" Height="100%" Orientation="Horizontal">
            ...
            <tlk:RadPane ID="RadPane2" runat="server" Scrolling="None">
                <tlk:RadGrid PageSize=50 ID="rgTerminate" runat="server" Height="100%" AllowMultiRowSelection="true">
                    ...
                    <MasterTableView DataKeyNames="ID,EMPLOYEE_CODE,TYPE_TERMINATE,CODE,EMPLOYEE_NAME,ORG_NAME,LAST_DATE,ORG_DESC" ClientDataKeyNames="ID,STATUS_CODE,IS_NOHIRE,STATUS_ID,EMPLOYEE_NAME,ORG_NAME,LAST_DATE">
                        <Columns>
...
<tlk:RadCodeBlock ID="RadCodeBlock1" runat="server">
    <script type="text/javascript">
        ...
        function GridCreated(sender, eventArgs) {
            registerOnfocusOut('RAD_SPLITTER_ctl00_MainContent_ctrlHU_Terminate_RadSplitter1');
        }

        function OpenNew() {
            window.open('/Default.aspx?mid=Profile&fid=ctrlHU_TerminateNewEdit&group=Business&FormType=0', "_self"); /*
            var pos = $("html").offset();
        ...
        function OpenEdit() {
            ...
            window.open('/Default.aspx?mid=Profile&fid=ctrlHU_TerminateNewEdit&group=Business&FormType=1&ID=' + id, "_self"); /*
            var pos = $("html").offset();
            oWindow.setSize(1020, $(window).height());
            oWindow.center(); */
        }
        ...
        function OpenView() {
            ...
            window.open('/Default.aspx?mid=Profile&fid=ctrlHU_TerminateNewEdit&group=Business&FormType=2&ID=' + id, "_self"); /*
            var pos = $("html").offset();
            oWindow.setSize(1020, $(window).height());
            oWindow.center(); */
        }
        ...
        function clientButtonClicking(sender, args) {
            ...
            if (args.get_item().get_commandName() == 'EXPORT' || args.get_item().get_commandName() == 'PRINT' || args.get_item().get_commandName() == 'KTK' || args.get_item().get_commandName() == 'HANDOVER') {
                enableAjax = false;
            }
            if (args.get_item().get_commandName() == "EDIT") {
                bCheck = CheckValidate();
                if (bCheck != 1 && bCheck != 3) {
                    OpenEdit();
                }
                if (bCheck == 1) {
                    m = '<%= Translate(CommonMessage.MESSAGE_NOT_SELECT_ROW) %>';
