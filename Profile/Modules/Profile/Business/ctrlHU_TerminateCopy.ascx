<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ctrlHU_TerminateCopy.ascx.vb"
    Inherits="Profile.ctrlHU_TerminateCopy" %>
        ...
        <tlk:RadSplitter ID="RadSplitter3" runat="server" Width="100%" Height="100%" Orientation="Horizontal">
            ...
            <tlk:RadPane ID="RadPane2" runat="server" Scrolling="None">
                <tlk:RadGrid PageSize=50 ID="rgTerminate" runat="server" Height="100%" AllowMultiRowSelection="true">
                    ...
                    <MasterTableView DataKeyNames="ID,EMPLOYEE_CODE,TYPE_TERMINATE,CODE,IS_COPY,ORG_DESC" ClientDataKeyNames="ID,STATUS_CODE,IS_NOHIRE,STATUS_ID,IS_COPY">
                        <Columns>
...
<tlk:RadCodeBlock ID="RadCodeBlock1" runat="server">
    <script type="text/javascript">
        ...
        function GridCreated(sender, eventArgs) {
            registerOnfocusOut('RAD_SPLITTER_ctl00_MainContent_ctrlHU_TerminateCopy_RadSplitter1');
        }
                
        function OpenEditWindow(states) {
            var empId = $find('<%= rgTerminate.ClientID%>').get_masterTableView().get_selectedItems()[0].getDataKeyValue('ID');
       ...
       <%-- function gridRowDblClick(sender, eventArgs) {
        ...
        function clientButtonClicking(sender, args) {
           var bCheck = $find('<%= rgTerminate.ClientID %>').get_masterTableView().get_selectedItems().length;
            ...
            var is_copy = $find('<%# rgTerminate.ClientID%>').get_masterTableView().get_selectedItems()[0].getDataKeyValue('IS_COPY');
            if (is_copy == -1){
                var m = 'Nhân viên nghỉ việc đã được sao chép';
                var n = noty({ text: m, dismissQueue: true, type: 'warning' });
                setTimeout(function () { $.noty.close(n.options.id); }, 10000);
                args.set_cancel(true);
                return;
