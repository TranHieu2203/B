        ...
        Try
            ...
            Select Case CurrentState
                ...
                Case CommonMessage.STATE_DELETE
                    If rep.DeleteAccident(rgTerminate.SelectedValue) Then
...
#Region "Event"
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_EXPORT
                    ...
                    Using xls As New ExcelCommon
                        ...
                        If dtData.Rows.Count > 0 Then
                            rgTerminate.ExportExcel(Server, Response, dtData, "Accident_list")
                        Else
    ...
    Private Sub Template_ImportAccident()
        ...
        Try
            ...
            ExportTemplate("Profile\Import\Import_BTTN.xls",
                                  dsData, Nothing, "Import_BTTN" & Format(Date.Now, "yyyymmdd"))
...
#Region "Custom"
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        Dim _filter As New AccidentDTO
        Dim rep As New ProfileBusinessRepository
        ...
        Try
            ...
            Else
                rgTerminate.DataSource = New List(Of AccidentDTO)
                Exit Function
            End If
            ...
            If rdFromLast.SelectedDate IsNot Nothing Then
                _filter.FROM_DATE = rdFromLast.SelectedDate
            End If
            If rdToLast.SelectedDate IsNot Nothing Then
                _filter.TO_DATE = rdToLast.SelectedDate
            End If
            Dim MaximumRows As Integer
            Dim Sorts As String = rgTerminate.MasterTableView.SortExpressions.GetSortString()
            If isFull Then
                If Sorts IsNot Nothing Then
                    Return rep.GetAccident(_filter, _param, Sorts).ToTable()
                Else
                    Return rep.GetAccident(_filter, _param).ToTable()
                End If
            Else
                If Sorts IsNot Nothing Then
                    rgTerminate.DataSource = rep.GetAccident(_filter, rgTerminate.CurrentPageIndex,
    ...
    Private Sub ctrlUpload1_OkClicked(sender As Object, e As EventArgs) Handles ctrlUpload1.OkClicked
        ...
        Try
            ...
            For Each rows As DataRow In dsDataPrepare.Tables(0).Select("EMPLOYEE_CODE<>'""'").CopyToDataTable.Rows
                ...
                newRow("ACCIDENT_DATE") = rows("ACCIDENT_DATE")
                newRow("INFORMATION") = rows("INFORMATION")
                newRow("REASON_ID") = If(IsNumeric(rows("REASON_ID")), rows("REASON_ID"), 0)
                newRow("COST") = rows("COST")

                newRow("TREATMENT_PLACE") = rows("TREATMENT_PLACE")
                newRow("MONEY") = rows("MONEY")
                newRow("MONEY_DATE") = rows("MONEY_DATE")
            ...
            If loadToGrid() Then
                ...
                DocXml = sw.ToString
                'Dim sp As New ProfileStoreProcedure()
                If sp.IMPORT_ACCIDENT(LogHelper.GetUserLog().Username.ToUpper, DocXml) Then
    ...
    Function loadToGrid() As Boolean
        ...
        Try
            ...
            For Each row As DataRow In dtData.Rows
                ...
                If row("ACCIDENT_DATE") Is DBNull.Value OrElse row("ACCIDENT_DATE") = "" Then
                    sError = "Chưa nhập ngày tai nạn"
                    ImportValidate.IsValidTime("ACCIDENT_DATE", row, rowError, isError, sError)
                Else
                    If IsDate(row("ACCIDENT_DATE")) = False Then
                        sError = "Ngày tai nạn không đúng định dạng"
                        ImportValidate.IsValidTime("ACCIDENT_DATE", row, rowError, isError, sError)
                    End If
                End If
VALIDATE:
                Dim empId = rep.CheckEmployee_Exits(row("EMPLOYEE_CODE"))
                If row("MONEY_DATE") IsNot DBNull.Value OrElse row("MONEY_DATE") <> "" Then
                    If IsDate(row("MONEY_DATE")) = False Then
                        sError = "Ngày nhận tiền không đúng định dạng"
                        ImportValidate.IsValidTime("MONEY_DATE", row, rowError, isError, sError)
                    End If
                ...
                If row("REASON_ID") Is DBNull.Value OrElse row("REASON_ID") = "0" Then
                    sError = "Chưa chọn nguyên nhân"
                    ImportValidate.IsValidTime("REASON_ID", row, rowError, isError, sError)
                End If

                If row("COST") Is DBNull.Value OrElse row("COST") = "" Then
                    sError = "Chưa nhập chi phí"
                    ImportValidate.IsValidTime("COST", row, rowError, isError, sError)
                ElseIf Not IsNumeric(row("COST")) Then
                    sError = "Chi phí không đúng định dạng"
                    ImportValidate.IsValidTime("COST", row, rowError, isError, sError)
                End If
                If row("INFORMATION") Is DBNull.Value OrElse row("INFORMATION") = "" Then
                    sError = "Chưa nhập Thông tin tai nạn"
                    ImportValidate.IsValidTime("INFORMATION", row, rowError, isError, sError)
                End If
                If row("TREATMENT_PLACE") Is DBNull.Value OrElse row("TREATMENT_PLACE") = "" Then
                    sError = "Chưa nhập Nơi chữa trị"
                    ImportValidate.IsValidTime("TREATMENT_PLACE", row, rowError, isError, sError)
                End If
                If Not IsNumeric(row("MONEY")) And row("MONEY") IsNot DBNull.Value Then
                    sError = "Tiền bồi thường không đúng định dạng"
                    ImportValidate.IsValidTime("MONEY", row, rowError, isError, sError)
                End If
            ...
            If dtError.Rows.Count > 0 Then
                Session("EXPORTREPORT_ACCIDENT") = dtError
                rep.Dispose()
                ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType(), "javascriptfunction", "ExportReport('TEMP_IMPORT_ACCIDENT');", True)
