...
Imports WebAppLog
Public Class ctrlHU_OccupationalSafety
...
#Region "Page"
    ...
    ''' </lastupdate>
    ''' <summary>
    ''' Ham khoi tao lai cac control cho man hinh
    ''' </summary>
    ''' <remarks></remarks>
...
#Region "Event"
    ...
    ''' Command them, sua, xoa tai an lao dong, xuat file excel
    ''' </summary>
                ...
                Case CommonMessage.TOOLBARITEM_EXPORT
                    ...
                    Using xls As New ExcelCommon
                        ...
                        ElseIf dtData.Rows.Count > 0 Then
                            rgData.ExportExcel(Server, Response, dtData, "Commend")
                        End If
                    End Using
            End Select
            _mylog.WriteLog(_mylog._info, _classPath, method, CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        ...
        Try
            If e.ActionName = CommonMessage.TOOLBARITEM_DELETE And e.ButtonID = MessageBoxButtonType.ButtonYes Then
                ...
                For Each item As GridDataItem In rgData.SelectedItems
                    ...
                    objD.Add(obj)
                Next
                If rep.DeleteOccupationalSafety(objD) Then
                    objD = Nothing
                    Refresh("UpdateView")
                End If
                rep.Dispose()
    ...
    ''' </lastupdate>
    ''' <summary>
    ''' Xu ly su kien NeedDataSource cho rgData
    ''' Tao du lieu filter
    ''' </summary>
...
#Region "Custom"
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        Dim _filter As New OccupationalSafetyDTO
        Dim rep As New ProfileBusinessRepository
        Dim startTime As DateTime = DateTime.UtcNow
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Try
            ...
            If isFull Then
                If Sorts IsNot Nothing Then
                    Return rep.GetOccupationalSafety(_filter, ctrlOrg.IsDissolve, Sorts).ToTable()
                Else
                    Return rep.GetOccupationalSafety(_filter, ctrlOrg.IsDissolve).ToTable()
                End If
            Else
                Dim lstOccupationalSafety As New List(Of OccupationalSafetyDTO)
                If Sorts IsNot Nothing Then
                    lstOccupationalSafety = rep.GetOccupationalSafety(_filter, ctrlOrg.IsDissolve, rgData.CurrentPageIndex, rgData.PageSize, MaximumRows, Sorts)
                Else
                    lstOccupationalSafety = rep.GetOccupationalSafety(_filter, ctrlOrg.IsDissolve, rgData.CurrentPageIndex, rgData.PageSize, MaximumRows)
                End If
                rgData.VirtualItemCount = MaximumRows
                rgData.DataSource = lstOccupationalSafety
            End If
            rep.Dispose()
            _mylog.WriteLog(_mylog._info, _classPath, method, CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
