            ...
            Else
                Select Case Message
                    Case "UpdateView"
                        ...
                        rgMain.Rebind()
                        ClearControlValue(txtCode, txtNameVN, txtNameEN, txtRemark)
                        CurrentState = CommonMessage.STATE_NORMAL
                    Case "InsertView"
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                    ...
                    Case "Cancel"
                        ...
                        ClearControlValue(txtNameVN, txtCode, txtNameEN, txtRemark)
                    Case ""
                End Select
            End If
            rep.Dispose()
        ...
        Try
            ...
            If isFull Then
                If Sorts IsNot Nothing Then
                    Return rep.GetChucvuTbl(_filter, Sorts).ToTable()
                Else
                    Return rep.GetChucvuTbl(_filter).ToTable()
                End If
            Else
                Dim ChucvuTbls As List(Of ChucvuDTO)
                If Sorts IsNot Nothing Then
                    ChucvuTbls = rep.GetChucvuTbl(_filter, rgMain.CurrentPageIndex, rgMain.PageSize, MaximumRows, Sorts)
                Else
                    ChucvuTbls = rep.GetChucvuTbl(_filter, rgMain.CurrentPageIndex, rgMain.PageSize, MaximumRows)
                End If
                rgMain.VirtualItemCount = MaximumRows
                rgMain.DataSource = ChucvuTbls
            End If
            rep.Dispose()
            _mylog.WriteLog(_mylog._info, _classPath, method, CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
        ...
        Try
            Select Case CurrentState
                Case CommonMessage.STATE_NEW
                    ...
                    EnableControlAll(True, txtCode, txtNameVN, txtNameEN, txtRemark)

                    Refresh("Cancel")
dontrefresh:
                Case CommonMessage.STATE_NORMAL
                    EnabledGridNotPostback(rgMain, True)
                    EnableControlAll(False, txtNameVN, txtCode, txtNameEN, txtRemark)
                Case CommonMessage.STATE_EDIT
                    EnabledGridNotPostback(rgMain, False)
                    EnableControlAll(True, txtNameVN, txtCode, txtNameEN, txtRemark)
                ...
                Case CommonMessage.STATE_DELETE
                    ...
                    If rep.DeleteChucvuTbl(lstDeletes) Then
                        Refresh("UpdateView")
                        UpdateControlState()
                    Else
                        CurrentState = CommonMessage.STATE_NORMAL
                ...
                Case CommonMessage.STATE_ACTIVE
                    ...
                    If rep.ActiveChucvuTbl(lstDeletes, "A") Then
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                        CurrentState = CommonMessage.STATE_NORMAL
                        rgMain.Rebind()
                    Else
                ...
                Case CommonMessage.STATE_DEACTIVE
                    ...
                    If rep.ActiveChucvuTbl(lstDeletes, "I") Then
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                        CurrentState = CommonMessage.STATE_NORMAL
                        rgMain.Rebind()
                    Else
...
#Region "Event"
    ...
    Protected Sub OnToolbar_Command(ByVal sender As Object, ByVal e As RadToolBarEventArgs) Handles Me.OnMainToolbarClick
        Dim objChucvuTbl As New ChucvuDTO
        Dim rep As New ProfileRepository
        Dim gID As Decimal
        Dim startTime As DateTime = DateTime.UtcNow
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_EXPORT
                    ...
                    Using xls As New ExcelCommon
                        ...
                        If dtData.Rows.Count > 0 Then
                            rgMain.ExportExcel(Server, Response, dtData, "ChucvuTbl")
                        Else
                ...
                Case CommonMessage.TOOLBARITEM_SAVE
                    If Page.IsValid Then
                        objChucvuTbl.CODE = txtCode.Text.Trim
                        objChucvuTbl.NAME_VN = txtNameVN.Text.Trim
                        objChucvuTbl.NAME_EN = txtNameEN.Text.Trim
                        objChucvuTbl.REMARK = txtRemark.Text.Trim
                        Select Case CurrentState
                            Case CommonMessage.STATE_NEW
                                objChucvuTbl.ACTFLG = "A"
                                If rep.InsertChucvuTbl(objChucvuTbl, gID) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
                                    IDSelect = gID
                                    Refresh("InsertView")
                                    UpdateControlState()
                            ...
                            Case CommonMessage.STATE_EDIT
                                objChucvuTbl.ID = rgMain.SelectedValue

                                Dim lst As New List(Of Decimal)
                                lst.Add(objChucvuTbl.ID)
                                If commonRes.CheckExistIDTable(lst, "HU_TITLE_TBL", "ID") Then
                                    ShowMessage(Translate(CommonMessage.MESSAGE_WARNING_EXIST_DATABASE), NotifyType.Warning)
                                    Exit Sub
                                End If

                                If rep.ModifyChucvuTbl(objChucvuTbl, gID) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
                                    IDSelect = objChucvuTbl.ID
                                    Refresh("UpdateView")
                                    UpdateControlState()
                                Else
                                    ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_FAIL), Utilities.NotifyType.Error)
    ...
    Private Sub cvalCode_ServerValidate(ByVal source As Object, ByVal args As System.Web.UI.WebControls.ServerValidateEventArgs) Handles cvalCode.ServerValidate
        Dim rep As New ProfileRepository
        Dim _validate As New ChucvuDTO
        Dim startTime As DateTime = DateTime.UtcNow
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        ...
        Try
            If CurrentState = CommonMessage.STATE_EDIT Then
                ...
                args.IsValid = rep.ValidateChucvuTblCode(_validate)
            Else
                _validate.CODE = txtCode.Text.Trim
                args.IsValid = rep.ValidateChucvuTblCode(_validate)
            End If
