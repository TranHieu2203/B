...
#Region "Page"
            ...
            Else
                Select Case Message
                    Case "UpdateView"
                        ...
                        rgMain.Rebind()
                        ClearControlValue(txtCode, txtName, cboTypeDocument, chkOriginal, chkAllowUpload)
                        CurrentState = CommonMessage.STATE_NORMAL
                    Case "InsertView"
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Dim rep As New ProfileRepository
        Dim _filter As New DocumentDTO
        Dim startTime As DateTime = DateTime.UtcNow
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        ...
        Try
            ...
            If isFull Then
                If Sorts IsNot Nothing Then
                    Return rep.GetAll_Document(_filter, Sorts).ToTable()
                Else
                    Return rep.GetAll_Document(_filter).ToTable()
                End If
            Else
                Dim Titles As List(Of DocumentDTO)
                If Sorts IsNot Nothing Then
                    Titles = rep.GetAll_Document(_filter, rgMain.CurrentPageIndex, rgMain.PageSize, MaximumRows, Sorts)
                Else
                    Titles = rep.GetAll_Document(_filter, rgMain.CurrentPageIndex, rgMain.PageSize, MaximumRows)
                End If
        ...
        Try
            Select Case CurrentState
                Case CommonMessage.STATE_NEW
                    ...
                    EnableControlAll(True, txtCode, txtName, cboTypeDocument, chkOriginal, chkAllowUpload)
                    Refresh("Cancel")
                Case CommonMessage.STATE_NORMAL
                    EnabledGridNotPostback(rgMain, True)
                    EnableControlAll(False, txtCode, txtName, cboTypeDocument, chkOriginal, chkAllowUpload)
                ...
                Case CommonMessage.STATE_DELETE
                    ...
                    If rep.DeleteDocument(lstDeletes) Then
                        Refresh("UpdateView")
                        UpdateControlState()
                    Else
                        CurrentState = CommonMessage.STATE_NORMAL
                ...
                Case CommonMessage.STATE_ACTIVE
                    ...
                    If rep.ActiveDocument(lstDeletes, "A") Then
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                        CurrentState = CommonMessage.STATE_NORMAL
                        rgMain.Rebind()
                    Else
                ...
                Case CommonMessage.STATE_DEACTIVE
                    ...
                    If rep.ActiveDocument(lstDeletes, "I") Then
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                        CurrentState = CommonMessage.STATE_NORMAL
                        rgMain.Rebind()
                    Else
    ...
    Public Overrides Sub BindData()
        ...
        Try
            ...
            Using rep As New ProfileRepository
                dataTable = rep.GetOtherList("DANG_TL", True)
                FillRadCombobox(cboTypeDocument, dataTable, "NAME", "ID")
            End Using

            Dim dic As New Dictionary(Of String, Control)
            dic.Add("CODE", txtCode)
            dic.Add("NAME_VN", txtName)
            dic.Add("TYPE_DOCUMENT_ID", cboTypeDocument)
            dic.Add("MUST_HAVE", chkOriginal)
            dic.Add("ALLOW_UPLOAD_FILE", chkAllowUpload)
...
#Region "Event"
    ...
    Protected Sub OnToolbar_Command(ByVal sender As Object, ByVal e As RadToolBarEventArgs) Handles Me.OnMainToolbarClick
        Dim objTitle As New DocumentDTO
        Dim rep As New ProfileRepository
        Dim gID As Decimal
        Dim startTime As DateTime = DateTime.UtcNow
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_SAVE
                    If Page.IsValid Then
                        ...
                        'GenerateTitleCode()
                        objTitle.CODE = txtCode.Text.ToUpper
                        objTitle.NAME_VN = txtName.Text
                        If cboTypeDocument.SelectedValue <> "" Then
                            objTitle.TYPE_DOCUMENT_ID = cboTypeDocument.SelectedValue
                        End If
                        objTitle.CHK_MUST_HAVE = If(chkOriginal.Checked = True, -1, 0)
                        objTitle.ALLOW_UPLOAD_FILE = If(chkAllowUpload.Checked = True, -1, 0)
                        objTitle.ACTFLG = "A"
                        Select Case CurrentState
                            Case CommonMessage.STATE_NEW
                                Dim check As Decimal
                                check = rep.Check_Exit_Document(objTitle)
                                If check = 1 Then
                                    ShowMessage(Translate("Mã tài liệu đã tồn tại"), NotifyType.Warning)
                                    Exit Sub
                                End If
                                If rep.InsertDocument(objTitle, gID) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
                                    IDSelect = gID
                                    Refresh("InsertView")
                                    UpdateControlState()
                            ...
                            Case CommonMessage.STATE_EDIT
                                ...
                                If rep.ModifyDocument(objTitle, gID) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
