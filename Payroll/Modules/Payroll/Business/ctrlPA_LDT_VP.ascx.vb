    ...
    Property dtData As DataTable
        Get
            If ViewState(Me.ID & "_dtData") Is Nothing Then
                ...
                dt.Columns.Add("MR", GetType(String))
                dt.Columns.Add("AVBA", GetType(String))
                dt.Columns.Add("RR6_A", GetType(String))
                dt.Columns.Add("BS", GetType(String))
                dt.Columns.Add("SPSG", GetType(String))
                dt.Columns.Add("D90", GetType(String))
                dt.Columns.Add("TA", GetType(String))
        ...
        Try
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_EXPORT_TEMPLATE
                    ...
                    ExportTemplate("Payroll\Import\Template_Import_LDT_VP.xls", _
                                              dsData, Nothing, _
    ...
    Private Sub ctrlUpload1_OkClicked(ByVal sender As Object, ByVal e As System.EventArgs) Handles ctrlUpload1.OkClicked
        ...
        Try
            ...
            For Each rows As DataRow In dsDataPrepare.Tables(0).Rows
                If String.IsNullOrEmpty(rows("YEAR").ToString) AndAlso String.IsNullOrEmpty(rows("MONTH").ToString) AndAlso String.IsNullOrEmpty(rows("EMPLOYEE_CODE").ToString) Then Continue For
                ...
                newRow("MR") = rows("MR")
                newRow("AVBA") = rows("AVBA")
                newRow("RR6_A") = rows("RR6_A")
                newRow("BS") = rows("BS")
                newRow("SPSG") = rows("SPSG")
                newRow("D90") = rows("D90")
                newRow("TA") = rows("TA")
                newRow("BILL") = rows("BILL")
            ...
            If loadToGrid() Then
                ...
                If sp.IMPORT_LDT_VP(DocXml, LogHelper.GetUserLog().Username.ToUpper) Then
...
#Region "Custom"
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Dim startTime As DateTime = DateTime.UtcNow
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Dim _filter As New PA_LDT_VPDTO
        Try
            If ctrlOrg.CurrentValue Is Nothing Then
                rgData.DataSource = New List(Of PA_LDT_VPDTO)
                Exit Function
            End If
            ...
            Dim lstObj As New List(Of PA_LDT_VPDTO)
            If isFull Then
                If Sorts IsNot Nothing Then
                    Return rep.GetLDT_VP(_filter, _param, Sorts).ToTable()
                Else
                    Return rep.GetLDT_VP(_filter, _param).ToTable()
                End If
            Else
                If Sorts IsNot Nothing Then
                    lstObj = rep.GetLDT_VP(_filter, rgData.CurrentPageIndex, rgData.PageSize, MaximumRows, _param, Sorts)
                Else
                    lstObj = rep.GetLDT_VP(_filter, rgData.CurrentPageIndex, rgData.PageSize, MaximumRows, _param)
                End If
    ...
    Function loadToGrid() As Boolean
        ...
        Try
            ...
            For Each row As DataRow In dtData.Rows
                ...
                sError = "Chưa nhập tháng"
                ImportValidate.EmptyValue("MONTH", row, rowError, isError, sError)
                ...
                If Not IsDBNull(row("YEAR")) AndAlso Not String.IsNullOrEmpty(row("YEAR")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("YEAR", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("MONTH")) AndAlso Not String.IsNullOrEmpty(row("MONTH")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("MONTH", row, rowError, isError, sError)
                    If IsNumeric(row("MONTH")) Then
                        sError = "Chỉ được nhập tháng từ 1-12"
                        If CDec(row("MONTH").ToString) <= 0 OrElse CDec(row("MONTH").ToString) > 12 Then
                            ImportValidate.IsValidTime("MONTH", row, rowError, isError, sError)
                        End If
                    End If
                End If

                If Not IsDBNull(row("MONTH")) AndAlso Not String.IsNullOrEmpty(row("MONTH")) AndAlso Not IsDBNull(row("YEAR")) AndAlso Not String.IsNullOrEmpty(row("YEAR")) Then
                    If IsNumeric(row("MONTH")) AndAlso IsNumeric(row("YEAR")) Then
                        sError = "Chỉ được nhập tháng từ 1-12"
                ...
                If Not IsDBNull(row("MR")) AndAlso Not String.IsNullOrEmpty(row("MR")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("MR", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("AVBA")) AndAlso Not String.IsNullOrEmpty(row("AVBA")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("AVBA", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("RR6_A")) AndAlso Not String.IsNullOrEmpty(row("RR6_A")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("RR6_A", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("BS")) AndAlso Not String.IsNullOrEmpty(row("BS")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("BS", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("SPSG")) AndAlso Not String.IsNullOrEmpty(row("SPSG")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("SPSG", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("D90")) AndAlso Not String.IsNullOrEmpty(row("D90")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("D90", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("TA")) AndAlso Not String.IsNullOrEmpty(row("TA")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("TA", row, rowError, isError, sError)
                End If

                If Not IsDBNull(row("BILL")) AndAlso Not String.IsNullOrEmpty(row("BILL")) Then
                    sError = "Chỉ được nhập số"
                    ImportValidate.IsValidNumber("BILL", row, rowError, isError, sError)
                End If
            ...
            If dtError.Rows.Count > 0 Then
                ...
                Session("LDT_VP_ERR") = dtError
                ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType(), "javascriptfunction", "ExportReport('Template_Import_LDT_VP_Error')", True)
