...
Public Class ctrlPA_EmpFormuler
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected WithEvents ctrlOrgPopupExport As ctrlFindOrgPopup
...
#Region "Property"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Property AT_SignDF As List(Of PA_EMP_FORMULER_DTO)
        Get
            Return ViewState(Me.ID & "_SignDF")
        End Get
        Set(ByVal value As List(Of PA_EMP_FORMULER_DTO))
...
#Region "Page"
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Dim obj As New PA_EMP_FORMULER_DTO
        Try
            ...
            Else
                rgWorkschedule.DataSource = New List(Of PA_EMP_FORMULER_DTO)
                Exit Function
            End If

            If Not isFull Then
                If Sorts IsNot Nothing Then
                    Me.AT_SignDF = rep.GetPA_EMP_FORMULER(obj, rgWorkschedule.CurrentPageIndex, rgWorkschedule.PageSize, MaximumRows, Sorts)
                Else
                    Me.AT_SignDF = rep.GetPA_EMP_FORMULER(obj, rgWorkschedule.CurrentPageIndex, rgWorkschedule.PageSize, MaximumRows)
                End If
                rgWorkschedule.VirtualItemCount = MaximumRows
                rgWorkschedule.DataSource = Me.AT_SignDF
            Else
                Return rep.GetPA_EMP_FORMULER(obj).ToTable
            End If
            rep.Dispose()
            _mylog.WriteLog(_mylog._info, _classPath, method, CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
        ...
        Try
            Select Case isLoadPopup
                ...
                Case 2
                    ...
                    ctrlOrgPopupExport = Me.Register("ctrlOrgPopup", "Common", "ctrlFindOrgPopup")
                    phPopup.Controls.Add(ctrlOrgPopupExport)
                    Exit Sub
                Case 3
            ...
            Select Case CurrentState
                Case CommonMessage.STATE_NEW
                    EnableControlAll(True, txtEmpCode, rdEffect_date, cboTiTle, cboGroupEmp, cboObjSal, cboForGroup, rdExpire_date, cboObjEmp, txtOrgName)
                ...
                Case CommonMessage.STATE_NORMAL
                    ...
                    ClearControlValue(txtEmpCode, txtEmpName, rdEffect_date, txtOrgName, cboTiTle, cboGroupEmp, cboObjSal, cboForGroup, rdExpire_date, cboObjEmp)
                    EnableControlAll(False, txtEmpCode, txtEmpName, rdEffect_date, txtOrgName, cboTiTle, cboGroupEmp, cboObjSal, cboForGroup, rdExpire_date, cboObjEmp)
                ...
                Case CommonMessage.STATE_DELETE
                    ...
                    Next
                    If rep.DeletePA_EMP_FORMULER(lstDeletes) Then
                        Refresh("UpdateView")
                        UpdateControlState()
                        rgWorkschedule.Rebind()
                    Else
    ...
    Public Overrides Sub BindData()
        ...
        Try
            ...
            dic.Add("OBJ_SAL_ID", cboObjSal)
            dic.Add("GROUP_EMPLOYEE_ID", cboGroupEmp)
            dic.Add("OBJECT_EMPLOYEE", cboObjEmp)
            dic.Add("ID", hidID)
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub ctrlOrgPopupExport_OrganizationSelected(ByVal sender As Object, ByVal e As Common.OrganizationSelectedEventArgs) Handles ctrlOrgPopupExport.OrganizationSelected
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Dim startTime As DateTime = DateTime.UtcNow

        Try
            ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType(), "javascriptfunction", "ExportReport('Template_PA_EmpFormuler&orgid= " & e.CurrentValue & "&IS_DISSOLVE=" & IIf(ctrlOrgPopupExport.IsDissolve, "1", "0") & "')", True)
            isLoadPopup = 0
...
#Region "Event"
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_SAVE
                    If Page.IsValid Then
                        ...
                        objSIGN.TITLE_ID = cboTiTle.SelectedValue
                        objSIGN.OBJ_SAL_ID = cboObjSal.SelectedValue
                        objSIGN.FOR_GROUP_ID = cboForGroup.SelectedValue
                        objSIGN.GROUP_EMPLOYEE_ID = cboGroupEmp.SelectedValue
                        objSIGN.EFFECT_DATE = rdEffect_date.SelectedDate
                        objSIGN.OBJECT_EMPLOYEE = cboObjEmp.SelectedValue
                        ...
                        Select Case CurrentState
                            Case CommonMessage.STATE_NEW
                                ...
                                If rep.CheckPA_EMP_FORMULERExits(hidEmpID.Value, hidOrgID.Value, cboTiTle.SelectedValue, cboGroupEmp.SelectedValue, cboForGroup.SelectedValue, rdEffect_date.SelectedDate, 0) Then
                                    ShowMessage(Translate("Dữ liệu nhân viên phong ban, chức danh, nhóm nhân viên, công thức lương và ngày hiệu lực đã tồn tại, Vui lòng kiểm tra lại."), Utilities.NotifyType.Error)
                                    Exit Sub
                                End If

                                If rep.InsertPA_EMP_FORMULER(objSIGN, gID) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
                                    IDSelect = gID
                                    Refresh("InsertView")
                                    UpdateControlState()
                                    ClearControlValue(txtEmpCode, txtEmpName, rdEffect_date, txtOrgName, cboTiTle, cboGroupEmp, cboObjSal, cboForGroup, rdExpire_date)
                                    rgWorkschedule.Rebind()
                                Else
                            ...
                            Case CommonMessage.STATE_EDIT
                                ...
                                If rep.CheckPA_EMP_FORMULERExits(hidEmpID.Value, hidOrgID.Value, cboTiTle.SelectedValue, cboGroupEmp.SelectedValue, cboForGroup.SelectedValue, rdEffect_date.SelectedDate, hidID.Value) Then
                                    ShowMessage(Translate("Dữ liệu nhân viên phong ban, chức danh, nhóm nhân viên, công thức lương và ngày hiệu lực đã tồn tại, Vui lòng kiểm tra lại."), Utilities.NotifyType.Error)
                                    Exit Sub
                                End If

                                If rep.ModifyPA_EMP_FORMULER(objSIGN, rgWorkschedule.SelectedValue) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
                                    IDSelect = objSIGN.ID
                                    Refresh("UpdateView")
                                    UpdateControlState()
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub ctrlOrgPopup_CancelClicked(ByVal sender As Object, ByVal e As System.EventArgs) Handles ctrlOrgPopup.CancelClicked, ctrlOrgPopupExport.CancelClicked
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Dim startTime As DateTime = DateTime.UtcNow
        Try
            isLoadPopup = 0
        ...
        Try
            ...
            If dtLogs Is Nothing Or dtLogs.Rows.Count <= 0 Then
                ...
                If ds.Tables(0) IsNot Nothing AndAlso ds.Tables(0).Rows.Count > 0 Then
                    ...
                    DocXml = sw.ToString
                    If rep.IMPORT_PA_EMP_FORMULER(DocXml) Then
    ...
    Private Sub TableMapping1(ByVal dtTemp As System.Data.DataTable)
        ...
        dtTemp.Columns(9).ColumnName = "EXPIRE_DATE"
        dtTemp.Columns(10).ColumnName = "ORG_ID"
        dtTemp.Columns(11).ColumnName = "TITLE_ID"
        dtTemp.Columns(12).ColumnName = "GROUP_EMPLOYEE_ID"
        dtTemp.Columns(13).ColumnName = "FOR_GROUP_ID"
        dtTemp.Columns(14).ColumnName = "OBJECT_EMPLOYEE"
        ...
        For Each rows As DataRow In dtTemp.Rows
            ...
            If Not IsNumeric(rows("TITLE_ID")) Then
                newRow("DISCIPTION") = newRow("DISCIPTION") + "Chức danh - Không đúng định dạng,"
            End If

            If Not IsNumeric(rows("OBJECT_EMPLOYEE")) Then
                newRow("DISCIPTION") = newRow("DISCIPTION") + "Nhóm nhân viên - Không đúng định dạng,"
            End If

            If Not IsNumeric(rows("FOR_GROUP_ID")) Then
                newRow("DISCIPTION") = newRow("DISCIPTION") + "Công thức lương - Không đúng định dạng,"
            End If

            If Not IsNumeric(rows("GROUP_EMPLOYEE_ID")) Then
                newRow("DISCIPTION") = newRow("DISCIPTION") + "Nhóm nhân viên - Không đúng định dạng,"
            End If
            ...
            If Not IsDBNull(rows("EXPIRE_DATE")) AndAlso CheckDate(rows("EXPIRE_DATE"), endDate) = False Then
                rows("EXPIRE_DATE") = "NULL"
                newRow("DISCIPTION") = newRow("DISCIPTION") + "Ngày hết hiệu lực - Không đúng định dạng,"
                _error = False
            End If

            If empId <> 0 AndAlso IsNumeric(rows("ORG_ID")) AndAlso IsNumeric(rows("TITLE_ID")) AndAlso IsNumeric(rows("GROUP_EMPLOYEE_ID")) AndAlso IsNumeric(rows("FOR_GROUP_ID")) AndAlso _error = True Then
                If rep1.CheckPA_EMP_FORMULERExits(empId, rows("ORG_ID"), rows("TITLE_ID"), rows("GROUP_EMPLOYEE_ID"), rows("FOR_GROUP_ID"), startDate, 0) Then
                    newRow("DISCIPTION") = newRow("DISCIPTION") + "Dữ liệu nhân viên phong ban, chức danh, nhóm nhân viên, công thức lương và ngày hiệu lực đã tồn tại,"
                    _error = False
                End If
            End If
...
#Region "Custom"
    ...
    Private Sub GetDataCombo()
        ...
        Try
            Using rep As New PayrollRepository
                Dim dtData = rep.GetObj_Sal()
                FillRadCombobox(cboObjSal, dtData, "NAME_VN", "ID")

                Dim dtData1 = rep.GetOtherList("STAFF_GROUP")
                FillRadCombobox(cboGroupEmp, dtData1, "NAME", "ID")

                dtData1 = rep.GetOtherList("OBJECT_EMPLOYEE")
                FillRadCombobox(cboObjEmp, dtData1, "NAME", "ID")
            End Using
    ...
    Private Sub cboObjSal_SelectedIndexChanged(sender As Object, e As Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs) Handles cboObjSal.SelectedIndexChanged
        If cboObjSal.SelectedValue <> "" Then
            Using rep As New PayrollRepository
                Dim dtData = rep.GetFORMULER_GROUP(cboObjSal.SelectedValue)
                FillRadCombobox(cboForGroup, dtData, "NAME_VN", "ID")
            End Using
        End If
    End Sub
    ...
    Private Sub cboTiTle_SelectedIndexChanged(sender As Object, e As Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs) Handles cboTiTle.SelectedIndexChanged
        If cboTiTle.SelectedValue <> "" Then
            Using rep As New PayrollRepository
                Dim groupEmpID = rep.GETGROUP_EMPLOYEE_ID(cboTiTle.SelectedValue)
                If groupEmpID IsNot Nothing Then
                    cboGroupEmp.SelectedValue = groupEmpID
                End If
            End Using
        End If
    End Sub
    ...
    Protected Sub rgDanhMuc_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles rgWorkschedule.SelectedIndexChanged
        ...
        Using rep As New ProfileRepository
            Dim dtData = rep.GetTitleByOrgID(data.ORG_ID, True)
            cboTiTle.ClearValue()
            cboTiTle.Items.Clear()
            FillRadCombobox(cboTiTle, dtData, "NAME", "ID")
            cboTiTle.SelectedValue = data.TITLE_ID
        End Using

        If data.OBJ_SAL_ID Then
            Using rep As New PayrollRepository
                Dim dtData1 = rep.GetFORMULER_GROUP(cboObjSal.SelectedValue)
                FillRadCombobox(cboForGroup, dtData1, "NAME_VN", "ID")
                cboForGroup.SelectedValue = data.FOR_GROUP_ID
            End Using
        End If
        ...
        Try
            ...
            If lstCommonEmployee.Count <> 0 Then
                ...
                For idx = 0 To lstCommonEmployee.Count - 1
                    ...
                    Using rep As New ProfileRepository
                        If IsNumeric(lstCommonEmployee(idx).ORG_ID) Then
                            dtData = rep.GetTitleByOrgID(lstCommonEmployee(idx).ORG_ID, True)
                            'cboTiTle.ClearValue()
                            'cboTiTle.Items.Clear()
                            FillRadCombobox(cboTiTle, dtData, "NAME", "ID")
                            cboTiTle.SelectedValue = lstCommonEmployee(idx).TITLE_ID

                            'dtData = rep.GetOtherList("STAFF_GROUP")
                            'FillRadCombobox(cboGroupEmp, dtData, "NAME", "ID")

                            cboGroupEmp.ClearValue()
                            If lstCommonEmployee(idx).GROUP_EMPLOYEE_ID IsNot Nothing Then
                                cboGroupEmp.SelectedValue = lstCommonEmployee(idx).GROUP_EMPLOYEE_ID
                            End If
