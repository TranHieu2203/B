<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ctrlOTRegByAnother.ascx.vb"
    Inherits="Attendance.ctrlOTRegByAnother
    " %>
        ...
        function clientButtonClicking(sender, args) {
            if (args.get_item().get_commandName() == "EDIT") {
                ...
                if (bCheck == 0) {
                    ...
                    OpenInNewTab('Default.aspx?mid=Attendance&fid=ctrlOTRegByAnotherNewEdit&id=' + id + '&typeUser=User&empId=' + empId);
                    args.set_cancel(true);
                }
        }
            ...
            if ($('#<%= hidValid.ClientID %>').val() == "1") {
                ...
                OpenInNewTab('Default.aspx?mid=Attendance&fid=ctrlOTRegByAnotherNewEdit&id=0&typeUser=User');
                args.set_cancel(true);
            }
...
function gridRowDblClick(sender, args) {
    var bCheck = $find('<%= rgMain.ClientID %>').get_masterTableView().get_selectedItems().length;
            if (bCheck == 0) {
                ...
                var id = $find('<%= rgMain.ClientID %>').get_masterTableView().get_selectedItems()[0].getDataKeyValue('ID')
                //var empId = $find('<%= rgMain.ClientID %>').get_masterTableView().get_selectedItems()[0].getDataKeyValue('EMPLOYEE_ID');
                OpenInNewTab('Default.aspx?mid=Attendance&fid=ctrlOTRegByAnotherNewEdit&id=' + id + '&typeUser=User');
                //args.set_cancel(true);
            }
    }
    ...
    function OpenInNewTab(url) {
        ...
        function SizeToFitMain() {
            ...
            $("#ctl00_MainContent_ctrlOTRegByAnother_RadSplitter1").stop().animate({ height: winH, width: winW }, 0);
            $("#ctl00_MainContent_ctrlOTRegByAnother_RadSplitter3").stop().animate({ height: winH, width: winW }, 0);
            $("#RAD_SPLITTER_PANE_CONTENT_ctl00_MainContent_ctrlOTRegByAnother_MainPane").stop().animate({ height: winH, width: winW }, 0);
           $("#ctl00_MainContent_ctrlOTRegByAnother_rgMain").stop().animate({ height: winH-76, width: winW }, 0);
            $("#RAD_SPLITTER_PANE_CONTENT_ctl00_MainContent_ctrlOTRegByAnother_RadPane2").stop().animate({ height: winH, width: winW }, 0);
