...
Public Class ctrlRC_Request_Person_TCTD
    ...
    Protected Sub InitControl()
        Try
            ...
            Common.Common.BuildToolbar(Me.MainToolBar, ToolbarItem.Edit, ToolbarItem.Export,
                                        ToolbarItem.Lock, ToolbarItem.Unlock)
            CType(MainToolBar.Items(0), RadToolBarButton).Text = "Cập nhật YCTD"
            CType(MainToolBar.Items(1), RadToolBarButton).Text = "Xuất Excel"
            CType(MainToolBar.Items(2), RadToolBarButton).Text = "Khóa"
            CType(MainToolBar.Items(3), RadToolBarButton).Text = "Mở Khóa"
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
        ...
        Try
            Select Case CurrentState
                ...
                Case CommonMessage.STATE_APPROVE
                    'Dim lstAp As New List(Of Decimal)
                    'For idx = 0 To rgData.SelectedItems.Count - 1
                    '    Dim item As GridDataItem = rgData.SelectedItems(idx)
                    '    lstAp.Add(item.GetDataKeyValue("ID"))
                    'Next
                    'If rep.UpdateStatusRequest(lstAp, RecruitmentCommon.RC_REQUEST_STATUS.APPROVE_ID) Then
                ...
                Case CommonMessage.STATE_ACTIVE
                    ...
                    Next
                    If store.UPDATE_RC_REQUEST_IS_LOCK(strIDCandidate, -1) Then
                        Dim lstAp As New List(Of Decimal)
                        For idx = 0 To rgData.SelectedItems.Count - 1
                            Dim item As GridDataItem = rgData.SelectedItems(idx)
                            lstAp.Add(item.GetDataKeyValue("ID"))
                        Next
                        If rep.UpdateStatusRequest(lstAp, RecruitmentCommon.RC_REQUEST_STATUS.APPROVE_ID, True) Then
                            ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                            CurrentState = CommonMessage.STATE_NORMAL
                            rgData.Rebind()
                        Else
                ...
                Case CommonMessage.STATE_DEACTIVE
                    ...
                    For Each dr As Telerik.Web.UI.GridDataItem In rgData.SelectedItems
                        strIDCandidate = strIDCandidate & dr.GetDataKeyValue("ID") & ","
                    Next
                    If store.UPDATE_RC_REQUEST_IS_LOCK(strIDCandidate, Nothing) Then
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                        CurrentState = CommonMessage.STATE_NORMAL
                        rgData.Rebind()
                    Else
        ...
        Try
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_LOCK
                    For Each item As GridDataItem In rgData.SelectedItems
                        If item.GetDataKeyValue("IS_CLOCK") = -1 Then
                            ShowMessage(Translate("YCTD đang trạng thái khóa"), NotifyType.Warning)
                            Exit Sub
                        End If
                    Next

                    ctrlMessageBox.MessageText = Translate("Bạn có muốn khóa YCTD này không")
                ...
                Case CommonMessage.TOOLBARITEM_UNLOCK
                    For Each item As GridDataItem In rgData.SelectedItems
                        If item.GetDataKeyValue("IS_CLOCK") <> -1 Then
                            ShowMessage(Translate("YCTD đang trạng thái chưa khóa"), NotifyType.Warning)
                            Exit Sub
                        End If
                    Next

                    ctrlMessageBox.MessageText = Translate("Bạn có muốn mở khóa YCTD này không")
...
#Region "Custom"
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Try
            ...
            'End If
            _filter.STATUS_ID = 4101 'phe duyet
            If store.CHECK_EXIST_SE_CONFIG("PERSONRE_TCTD") = -1 Then
                _filter.IS_INSERT_PRO = True
            Else
                _filter.IS_INSERT_PRO = False
            End If
            _filter.PERSON_PT_RC = store.GET_EMP_BY_USER(UserLogHelper.CurrentLogUser.ID)
        ...
        Try
            ...
            If loadToGrid() Then
                ...
                For Each dr In dsDataComper.Rows
                    ...
                    dto.LOCATION_ID = If(dr("Work_Location_ID") <> "", CDec(dr("Work_Location_ID")), Nothing)
                    dto.SEND_DATE = ToDate(dr("SEND_DATE"))
                    dto.EXPECTED_JOIN_DATE = ToDate(dr("EXPECTED_JOIN_DATE"))
                    dto.CONTRACT_TYPE_ID = If(dr("LABOR_TYPE_ID") <> "", CDec(dr("LABOR_TYPE_ID")), Nothing)
