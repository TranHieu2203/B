...
Public Class ctrlDashboardPlanning
...
#Region "Custom"
    ...
    Protected Sub CreateDataFilter()
        ...
        Try
            ...
            'End If
            'Dim _param = New ParamDTO With {.ORG_ID = Decimal.Parse(0),
            '                                   .IS_DISSOLVE = ctrlOrg.IsDissolve}

            '_filter.YEAR = rntxtYear.Value
            'dsData = rep.GetSalaryEmpTracker(_filter, _param)
            dsData = store.GET_RANK_SAL_BY_YEAR(LogHelper.CurrentUser.USERNAME.ToUpper, rntxtYear.Value, Decimal.Parse(ctrlOrg.CurrentValue), ctrlOrg.IsDissolve)
            RefreshBar(dsData)
        Catch ex As Exception
            Throw ex
        End Try
        ...
        Try
            ...
            For index As Integer = 0 To dsData.Tables(1).Rows.Count - 1
                If index = dsData.Tables(1).Rows.Count - 1 Then
                    data &= dsData.Tables(1).Rows(index)("SAL_VALUE")
                Else
                    data &= dsData.Tables(1).Rows(index)("SAL_VALUE") & ","
                End If
            Next
            ...
            For index As Integer = 0 To dsData.Tables(2).Rows.Count - 1
                If index = dsData.Tables(2).Rows.Count - 1 Then
                    data1 &= dsData.Tables(2).Rows(index)("TOTAL_DINHBIEN")
                Else
                    data1 &= dsData.Tables(2).Rows(index)("TOTAL_DINHBIEN") & ","
                End If
            Next
            ...
            For index As Integer = 0 To dsData.Tables(3).Rows.Count - 1
                If index = dsData.Tables(3).Rows.Count - 1 Then
                    data1 &= dsData.Tables(3).Rows(index)("SAL_VALUE")
                Else
                    data1 &= dsData.Tables(3).Rows(index)("SAL_VALUE") & ","
                End If
            Next
