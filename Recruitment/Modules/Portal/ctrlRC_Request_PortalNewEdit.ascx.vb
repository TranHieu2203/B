...
Public Class ctrlRC_Request_PortalNewEdit
    Inherits CommonView
        ...
        Try
            Select Case Message
                Case "UpdateView"
                    ...
                    Dim obj = rep.GetRequestPortalByID(New RequestPortalDTO With {.ID = IDSelect})
                    If obj.IS_APPROVED_CODE.Trim.ToUpper <> "R" Then
                    ...
                    End If
                    'Else
                    '    ShowMessage(Translate("Không được chỉnh sửa"), Utilities.NotifyType.Warning)
                ...
                Case "InsertView"
                    ...
                    If Not IsPostBack Then
                        ...
                        Dim Employee As New CommonBusiness.EmployeePopupFindDTO

                        If Request.Params("State").Trim.ToString.ToUpper = "NEW" Then
                            Dim log = LogHelper.CurrentUser
                            Employee.EMPLOYEE_ID = log.EMPLOYEE_ID
                            Employee.FULLNAME_VN = log.EMPLOYEE_FULLNAME_VN
                            Employee.EMPLOYEE_CODE = log.EMPLOYEE_CODE
                            lstCommonEmployee.Add(Employee)
                        Else
                ...
                Case "CopyRequest"
                    ...
                    Dim obj = rep.GetRequestPortalByID(New RequestPortalDTO With {.ID = Decimal.Parse(Request.Params("COPY_REQUEST_ID"))})
                    hidOrgID.Value = obj.ORG_ID.ToString()
                    txtOrgName.Text = obj.ORG_NAME
                    If obj.ORG_DESC IsNot Nothing AndAlso obj.ORG_DESC <> "" Then
                        txtOrgName.ToolTip = DrawTreeByString(obj.ORG_DESC)
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARITEM_SAVE
                    If Page.IsValid Then
                        Dim objRequest As New RequestPortalDTO
                        Dim lstEmp As New List(Of RequestEmpPortalDTO)
                        ...
                        For Each item In Employee_list
                            Dim obj As New RequestEmpPortalDTO
                            obj.EMPLOYEE_ID = item.ID
                            lstEmp.Add(obj)
                        Next
                        objRequest.lstEmp = lstEmp
                        objRequest.IS_INSERT_PRO = IS_INSERT_PROGRAM
                        Select Case CurrentState
                            Case CommonMessage.STATE_NEW
                                If rep.InsertRequestPortal(objRequest, gID) Then
                                    ''POPUPTOLINK
                                    Response.Redirect("/Default.aspx?mid=Recruitment&fid=ctrlRC_Request_Portal")
                            ...
                            Case CommonMessage.STATE_EDIT
                                ...
                                If rep.ModifyRequestPortal(objRequest, gID) Then
                                    ''POPUPTOLINK
                                    Response.Redirect("/Default.aspx?mid=Recruitment&fid=ctrlRC_Request_Portal")
...
#Region "Custom"
        ...
        Try
            If CurrentState Is Nothing Then
                ...
                If Request.Params("orgid") IsNot Nothing Then
                    IDOrg = Decimal.Parse(Request.Params("orgid"))
                    hidOrgID.Value = IDOrg
                    LoadComboTitle()
                End If
                If Request.Params("Org_Name") IsNot Nothing Then
                    txtOrgName.Text = Request.Params("Org_Name").ToString()
                End If
                If IDSelect IsNot Nothing Then
                    Refresh("UpdateView")
                ElseIf Request.Params("COPY_REQUEST_ID") IsNot Nothing Then
                    Refresh("CopyRequest")
                Else
                    Refresh("InsertView")
                End If
    ...
    Private Sub CheckNumRecruit()
        Try
            If Not cboRecruitReason.SelectedValue <> "" Then
                rntxtRecruitNumber.Value = Nothing
                ShowMessage(Translate("Bạn cần chọn hình thức tuyển dụng trước khi nhập số lượng cần tuyển"), Utilities.NotifyType.Warning)
                Exit Sub
            End If
    ...
    Protected Sub cboTitle_SelectedIndexChanged(ByVal sender As Object, ByVal e As Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs) Handles cboTitle.SelectedIndexChanged
        ...
        'End If
        If hidID.Value <> "" Then
            Dim obj = rep.GetRequestPortalByID(New RequestPortalDTO With {.ID = Decimal.Parse(hidID.Value)})
    ...
    Protected Sub GenerateYCTD()
        Try
            ClearControlValue(txtCode_YCTD, txtName_YCTD)
            If IsDate(rdSendDate.SelectedDate) AndAlso hidOrgID.Value <> String.Empty AndAlso cboTitle.SelectedValue <> "" Then
    ...
    Private Sub CalDifferenceNumber()
        Try
            ...
            If cboRecruitReason.SelectedValue <> "" Then
                ...
                End If
            Else
                If txtPayrollLimit.Value - txtCurrentNumber.Value < 0 Then
                    ShowMessage(Translate("Số lượng định biên không được bé hơn Số lượng nhân viên đang làm việc"), Utilities.NotifyType.Warning)
