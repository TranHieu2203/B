...
Public Class ctrlOTRegByAnother
    Inherits CommonView
            ...
            Select Case CurrentState
                ...
                Case CommonMessage.STATE_APPROVE
                    ...
                    If rgMain.SelectedItems.Count = 1 Then
                        Dim item As GridDataItem = DirectCast(rgMain.SelectedItems(0), GridDataItem)
                        Dim dto As New AT_OT_REGISTRATIONDTO
                        dto.ID = item.GetDataKeyValue("ID")
                        dto.REGIST_DATE = item.GetDataKeyValue("REGIST_DATE")
                        Dim _emp_ID = item.GetDataKeyValue("EMPLOYEE_ID")
                        'Kiem tra ky cong da dong hay chua
                        Dim periodid = rep.GetperiodID_2(_emp_ID, dto.REGIST_DATE)
                        If periodid = 0 Then
...
#Region "Custom"
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Try
            ...
            _filter.CREATED_BY_EMP = EmployeeID
            ...
            If Not isFull Then
                If Sorts IsNot Nothing Then
                    dtData = rep.GetPortalOtRegByAnotherData(_filter, MaximumRows, rgMain.CurrentPageIndex, rgMain.PageSize, "EMPLOYEE_CODE, REGIST_DATE desc")
                Else
                    dtData = rep.GetPortalOtRegByAnotherData(_filter, MaximumRows, rgMain.CurrentPageIndex, rgMain.PageSize)
                End If
            Else
                Return rep.GetPortalOtRegByAnotherData(_filter)
            End If
            If Not IsPostBack Then
                DesignGrid(dtData)
            End If
            ...
            If Not column.ColumnName.Contains("ID") And column.ColumnName <> "STATUS" And
             column.ColumnName <> "CBSTATUS" And Not column.ColumnName.Contains("DATE") And Not column.ColumnName.Contains("CHECK") Then
                ...
                If column.ColumnName = "STT" OrElse column.ColumnName = "MODIFIED_BY" OrElse column.ColumnName = "WORK_STATUS" _
                    OrElse column.ColumnName = "IS_DELETED" OrElse column.ColumnName = "STT" OrElse column.ColumnName.ToUpper = "MODIFIED_NAME" Then
                    rCol.Visible = False
                End If
