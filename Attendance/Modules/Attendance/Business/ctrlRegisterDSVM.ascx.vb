...
#Region "Page"
    ...
    Public Overrides Sub BindData()
        ...
        Try
            ...
            '    End If
            'End If
            dtData = rep.GetOtherList("DTVS", True)
            FillRadCombobox(cboTypeDMVS, dtData, "NAME", "ID")
            ...
            Select Case CurrentState
                Case "APPROVE_REG"
                    ...
                    For idx = 0 To rglateCom.SelectedItems.Count - 1
                        ...
                        lstID.Add(lst)
                    Next
                    If rep.ApproveApp(lstID, 1, LogHelper.CurrentUser.USERNAME, "DMVS") Then
                        For idx = 0 To rglateCom.SelectedItems.Count - 1
                            Dim id = 0
                            Dim item As GridDataItem = rglateCom.SelectedItems(idx)

                            id = item.GetDataKeyValue("ID")
                            Dim id_sw = item.GetDataKeyValue("ID_AT_SWIPE")
                            Dim wk = item.GetDataKeyValue("WORKINGDAY")
                            Dim empId = item.GetDataKeyValue("EMPLOYEE_ID")

                            store.UPDATE_INSERT_AT_SWIPE_DATA(id, id_sw)
                            store.CAL_TIME_TIMESHEET_EMP(LogHelper.GetUserLog.Username, wk, empId)
                        Next
                ...
                Case CommonMessage.STATE_DELETE
                    ...
                    Next
                    Dim lst_swipe = store.GET_SWIPE_DATA(lstID).Rows(0)(0).ToString
                    If rep.DeleteLate_combackout(lstDeletes) Then
                        Dim re = store.DELETE_SWIPE_DATA(lst_swipe)

                        'TÍNH LẠI BẢNG CÔNG GỐC
                        For idx = 0 To rglateCom.SelectedItems.Count - 1
                            Dim itm As GridDataItem = rglateCom.SelectedItems(idx)
                            store.CAL_TIME_TIMESHEET_EMP(LogHelper.GetUserLog.Username, itm.GetDataKeyValue("WORKINGDAY"), itm.GetDataKeyValue("EMPLOYEE_ID"))
                        Next
...
#Region "Event"
    ...
    Function SaveDSVM() As Boolean
        ...
        Try
            ...
            For Each row As DataRow In dtDataImportEmployee.Rows
                ...
                'obj.ORG_CHECK_IN = Decimal.Parse(row("ORG_CHECK_IN"))
                obj.REGIST_INFO = Decimal.Parse(row("REGIST_INFO"))
                obj.REASON_ID = Decimal.Parse(row("REASON_ID"))
    ...
    Function loadToGrid() As Boolean
        ...
        Try
            ...
            For Each row As DataRow In dtData.Rows
                ...
                sError = "Lý do"
                ImportValidate.IsValidList("REASON_NAME", "REASON_ID", row, rowError, isError, sError)
                sError = "Chưa nhập lý do chi tiết"
                ImportValidate.EmptyValue("REMARK", row, rowError, isError, sError)
                ...
                If row("TO_HOURS") <> "" AndAlso row("FROM_HOURS") <> "" AndAlso isError = False Then
                    If Convert.ToDateTime(row("TO_HOURS")) < Convert.ToDateTime(row("FROM_HOURS")) Then
                        isError = True
                ...
                For j As Integer = 0 To dtData.Rows.Count - 1
                    If dtData(j)("EMPLOYEE_ID") Is DBNull.Value Then
                        ...
                        Else
                            ...
                            obj.EMPLOYEE_ID = Convert.ToDecimal(dtData(j)("EMPLOYEE_ID"))
                            obj.WORKINGDAY = Convert.ToDateTime(dtData(j)("WORKINGDAY"))
                            If rep.ValidateLate_combackout(obj) Then
                                rowError("WORKINGDAY") = "Ngày đăng ký đã tồn tại trong hệ thống"
                                isError = True
                            End If
                        End If
                    End If
        ...
        Get
            ...
            Try
                ...
                If ViewState(Me.ID & "_dtData") Is Nothing Then
                    ...
                    dt.Columns.Add("ORG_CHECK_IN", GetType(String))
                    ...
                    dt.Columns.Add("REASON_ID", GetType(String))
                    dt.Columns.Add("REASON_NAME", GetType(String))
    ...
    Private Sub ctrlUpload1_OkClicked(ByVal sender As Object, ByVal e As System.EventArgs) Handles ctrlUpload1.OkClicked
        ...
        Try
            ...
            For Each rows As DataRow In dsDataPrepare.Tables(0).Rows
                If String.IsNullOrEmpty(rows("EMPLOYEE_CODE").ToString) Then Continue For
                Dim newRow As DataRow = dtData.NewRow
                newRow("EMPLOYEE_CODE") = rows("EMPLOYEE_CODE")
                newRow("VN_FULLNAME") = rows("VN_FULLNAME")
                newRow("TITLE_NAME") = rows("TITLE_NAME")
                newRow("ORG_NAME") = rows("ORG_NAME")
                'newRow("ORG_PATH") = rows("ORG_PATH")
                newRow("WORKINGDAY") = rows("WORKINGDAY")
                newRow("FROM_HOURS") = rows("FROM_HOURS")
                newRow("TO_HOURS") = rows("TO_HOURS")
                newRow("TYPE_DMVS_ID") = rows("TYPE_DMVS_ID")
                newRow("TYPE_DMVS_NAME") = rows("TYPE_DMVS_NAME")
                'newRow("ORG_CHECK_IN") = rows("ORG_CHECK_IN")
                'newRow("ORG_CHECK_IN_NAME") = rows("ORG_CHECK_IN_NAME")
                newRow("REMARK") = rows("REMARK")
                newRow("REGIST_INFO") = rows("REGIST_INFO")
                newRow("REASON_ID") = rows("REASON_ID")
                newRow("REASON_NAME") = rows("REASON_NAME")
                dtData.Rows.Add(newRow)
            Next
            If loadToGrid() = False Then
                ShowMessage(Translate("Import không thành công, vui lòng kiểm trả lại file mẫu"), NotifyType.Warning)
                Exit Sub
            Else
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Try
            ...
            End If
            If cboTypeDMVS.SelectedValue <> "" Then
                obj.TYPE_DMVS_ID = cboTypeDMVS.SelectedValue
            End If
            If cboStatus.SelectedValue <> "" Then
                obj.STATUS = cboStatus.SelectedValue
            End If
            If cboObjectEmployee.SelectedValue <> "" Then
                obj.WORK_STATUS = cboObjectEmployee.SelectedValue
            End If
