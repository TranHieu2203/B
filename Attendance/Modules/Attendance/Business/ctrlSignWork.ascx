...
function OpenEditWindow() {
    ...
    var cbo1 = $find("<%# cboEmpObj.ClientID%>");
    var EmpObj = cbo1.get_value();
    if (EmpObj.length = 0) {
        m = '<%# Translate("Bạn phải chọn đối tượng nhân viên.")%>';
                var n = noty({ text: m, dismissQueue: true, type: 'warning' });
                setTimeout(function () { $.noty.close(n.options.id); }, 5000);
                return;
            }

    var empID = $find('<%# rgSignWork.ClientID%>').get_masterTableView().get_selectedItems()[0].getDataKeyValue('ID');
    var oWindow = radopen('Dialog.aspx?mid=Attendance&fid=ctrlSignWorkEdit&group=Business&periodid=' + periodID + '&EmpObj=' + EmpObj + '&empID=' + empID, "rwPopup");
