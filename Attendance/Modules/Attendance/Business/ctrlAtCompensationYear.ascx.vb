...
Public Class ctrlAtCompensationYear
...
#Region "Properties"
    ...
    Public Property Entitlement As List(Of AT_COMPENSATION_YEARDTO)
        Get
            Return ViewState(Me.ID & "_Entitlement_DTO")
        End Get
        Set(ByVal value As List(Of AT_COMPENSATION_YEARDTO))
    ...
    Property dtData As DataTable
        Get
            If ViewState(Me.ID & "_dtData") Is Nothing Then
                ...
                dt.Columns.Add("YEAR_NB", GetType(String))
                dt.Columns.Add("NB_TRANFER", GetType(String))
                dt.Columns.Add("NB_EDIT", GetType(String))
        ...
        Try
            ...
            Select Case CurrentState
                Case CommonMessage.STATE_DELETE
                    ...
                    Next
                    If rep.DeleteCompensationYear(lstDeletes) Then
                        Refresh("UpdateView")
                        UpdateControlState()
                    Else
                        CurrentState = CommonMessage.STATE_NORMAL
...
#Region "Event"
        ...
        Try
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case TOOLBARTIEM_CALCULATE
                    ...
                    If rep.CAL_COMPENSATION_YEAR(_param, cboYear.SelectedValue) Then
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                        Refresh("UpdateView")
                    Else
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_FAIL), NotifyType.Error)
                ...
                Case TOOLBARITEM_EXPORT
                    Using xls As New ExcelCommon
                        ...
                        If dtDatas.Rows.Count > 0 Then
                            rgEntitlement.ExportExcel(Server, Response, dtDatas, "DataEntitlementNB")
                        End If
                    End Using
                Case TOOLBARITEM_SYNC
                    Dim rep As New AttendanceRepository
                    Dim lstData As New List(Of AT_COMPENSATION_YEARDTO)
                    Dim data As AT_COMPENSATION_YEARDTO
                    For Each row As GridDataItem In rgEntitlement.SelectedItems
                        data = New AT_COMPENSATION_YEARDTO
                        data.ID = row.GetDataKeyValue("ID")
                        data.YEAR_NB = Decimal.Parse((TryCast(row.FindControl("txtYEAR_NB"), RadTextBox).Text).Replace(".", ","))
                        data.NB_TRANFER = Decimal.Parse((TryCast(row.FindControl("txtNB_TRANFER"), RadTextBox).Text).Replace(".", ","))
                        data.NB_EDIT = Decimal.Parse((TryCast(row.FindControl("txtNB_EDIT"), RadTextBox).Text).Replace(".", ","))

                        lstData.Add(data)
                    Next

                    Dim count As Integer = 0
                    For Each item As AT_COMPENSATION_YEARDTO In lstData
                        If rep.UPDATE_COMPENSATION_YEAR(item.ID, item.YEAR_NB, item.NB_TRANFER, item.NB_EDIT) Then
                            count = count + 1
                        End If
                    Next
                ...
                Case CommonMessage.TOOLBARITEM_EXPORT_TEMPLATE
                    ExportTemplate("Attendance\Import\Template_import_Year_Compenstion.xls",
                                              Nothing, Nothing,
                                              "Template_import_Year_Compenstion" & Format(Date.Now, "yyyyMMdd"))
    ...
    Private Function CreateDataFilter(Optional ByVal isFull As Boolean = False)
        ...
        Dim startTime As DateTime = DateTime.UtcNow
        Dim rep As New AttendanceRepository
        Dim obj As New AT_COMPENSATION_YEARDTO
        Try
            ...
            If Not isFull Then
                If Sorts IsNot Nothing Then
                    Me.Entitlement = rep.GetAtCompensation(obj, _param, MaximumRows, rgEntitlement.CurrentPageIndex, rgEntitlement.PageSize, "CREATED_DATE desc")
                Else
                    Me.Entitlement = rep.GetAtCompensation(obj, _param, MaximumRows, rgEntitlement.CurrentPageIndex, rgEntitlement.PageSize)
                End If
            Else
                Return rep.GetAtCompensation(obj, _param).ToTable()
            End If
    ...
    Private Sub ctrlUpload1_OkClicked(ByVal sender As Object, ByVal e As System.EventArgs) Handles ctrlUpload1.OkClicked
        ...
        Try
            ...
            For Each rows As DataRow In dsDataPrepare.Tables(0).Rows
                ...
                newRow("YEAR_NB") = rows("YEAR_NB")
                newRow("NB_TRANFER") = rows("NB_TRANFER")
                newRow("NB_EDIT") = rows("NB_EDIT")
            ...
            If loadToGrid() Then
                ...
                DocXml = sw.ToString
                Dim rep As New AttendanceRepository
                If rep.IMPORT_AT_COMPENSATION_YEAR(DocXml) Then
    ...
    Function loadToGrid() As Boolean
        ...
        Try
            ...
            For Each row As DataRow In dtData.Rows
                ...
                sError = "Chưa nhập Số phép bù còn lại"
                ImportValidate.EmptyValue("YEAR_NB", row, rowError, isError, sError)

                sError = "Chưa nhập Số phép bù chuyển sang"
                ImportValidate.EmptyValue("NB_TRANFER", row, rowError, isError, sError)

                sError = "Chưa nhập Số phép bù điều chỉnh"
                ImportValidate.EmptyValue("NB_EDIT", row, rowError, isError, sError)

                sError = "Năm phải là kiểu số"
                ImportValidate.IsValidNumber("YEAR", row, rowError, isError, sError)

                sError = "Số phép bù còn lại phải là kiểu số"
                ImportValidate.IsValidNumber("YEAR_NB", row, rowError, isError, sError)

                sError = "Số phép bù chuyển sang phải là kiểu số"
                ImportValidate.IsValidNumber("NB_TRANFER", row, rowError, isError, sError)

                sError = "Số phép bù điều chỉnh phải là kiểu số"
                ImportValidate.IsValidNumber("NB_EDIT", row, rowError, isError, sError)

                If Not IsDBNull(row("YEAR")) AndAlso Not String.IsNullOrEmpty(row("YEAR")) Then
                    If IsNumeric(row("YEAR")) AndAlso (CDec(row("YEAR")) < 1900 Or CDec(row("YEAR")) > 3000) Then
            ...
            If dtError.Rows.Count > 0 Then
                ...
                Session("AT_COMPEN_YEAR_ERR") = dtError
                ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType(), "javascriptfunction", "ExportReport('Template_import_Year_Compenstion_Error')", True)
