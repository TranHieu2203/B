...
Public Class ctrlAtAdvanceLeaveEmp
...
#Region "Property"
    ...
    Public Property At_Holiday As List(Of AT_ADVANCE_LEAVE_EMPDTO)
        Get
            Return ViewState(Me.ID & "_Holiday")
        End Get
        Set(ByVal value As List(Of AT_ADVANCE_LEAVE_EMPDTO))
...
#Region "Page"
    ...
    Protected Function CreateDataFilter(Optional ByVal isFull As Boolean = False) As DataTable
        ...
        Dim obj As New AT_ADVANCE_LEAVE_EMPDTO
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Try
            Dim startTime As DateTime = DateTime.UtcNow
            ...
            If Not isFull Then
                ...
                If Sorts IsNot Nothing Then
                    Me.At_Holiday = rep.GetAtAdvanceLeaveEmp(obj, rgDanhMuc.CurrentPageIndex, rgDanhMuc.PageSize, MaximumRows, Sorts)
                Else
                    Me.At_Holiday = rep.GetAtAdvanceLeaveEmp(obj, rgDanhMuc.CurrentPageIndex, rgDanhMuc.PageSize, MaximumRows)
                End If
                rgDanhMuc.VirtualItemCount = MaximumRows
                rgDanhMuc.DataSource = Me.At_Holiday
            Else
                Return rep.GetAtAdvanceLeaveEmp(obj).ToTable
            End If
            _myLog.WriteLog(_myLog._info, _classPath, method,
                                        CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
        ...
        Try
            ...
            Select Case CurrentState
                Case CommonMessage.STATE_NORMAL
                    ClearControlValue(txtEmployeeCode, txtEmployeeName, txtOrgName, rdNote, txtTitleName, rdEffectDate, ntxtAdvanceLeave)
                    EnableControlAll(False, txtEmployeeCode, rdEffectDate, ntxtAdvanceLeave, rdNote)
                    btnFindEmployee.Enabled = False
                    EnabledGridNotPostback(rgDanhMuc, True)
                Case CommonMessage.STATE_EDIT

                    btnFindEmployee.Enabled = True
                    EnableControlAll(True, txtEmployeeCode, rdEffectDate, ntxtAdvanceLeave, rdNote)
                ...
                Case CommonMessage.STATE_DELETE
                    ...
                    Next
                    If rep.DeleteAtAdvanceLeaveEmp(lstDeletes) Then
                        Refresh("UpdateView")
                        UpdateControlState()
                    Else
                        CurrentState = CommonMessage.STATE_NORMAL
    ...
    Public Overrides Sub BindData()
        ...
        Try
            ...
            dic.Add("ADVANCELEAVE", ntxtAdvanceLeave)
            dic.Add("NOTE", rdNote)
...
#Region "Event"
    ...
    Protected Sub OnToolbar_Command(ByVal sender As Object, ByVal e As RadToolBarEventArgs) Handles Me.OnMainToolbarClick
        Dim objHoliday As New AT_ADVANCE_LEAVE_EMPDTO
        Dim rep As New AttendanceRepository
        Dim gID As Decimal
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Try
                ...
                Case CommonMessage.TOOLBARITEM_SAVE
                    If Page.IsValid Then
                        ...
                        If rdEffectDate.SelectedDate IsNot Nothing Then
                            objHoliday.FROMDATE_EFFECT = rdEffectDate.SelectedDate
                        End If

                        If ntxtAdvanceLeave.Value IsNot Nothing Then
                            objHoliday.ADVANCELEAVE = ntxtAdvanceLeave.Value
                        End If

                        If CommonConfig.AT_ADVANCELEAVE() AndAlso CommonConfig.AT_ADVANCELEAVE_VALUE() < objHoliday.ADVANCELEAVE Then
                            ShowMessage(Translate("Số phép tạm ứng vượt quá quy định cho ứng phép."), Utilities.NotifyType.Warning)
                            Exit Sub
                        End If

                        Dim dt As DataTable = rep.GET_AT_SETUPELEAVE_YEAR(rdEffectDate.SelectedDate.Value.Year)

                        Dim dt1 As DataTable = rep.SUM_AT_ADVANCELEAVE_EMP_YEAR(objHoliday.EMPLOYEE_ID, rdEffectDate.SelectedDate.Value.Year, If(rgDanhMuc.SelectedValue Is Nothing, 0, rgDanhMuc.SelectedValue))

                        If dt Is Nothing Then
                            ShowMessage(Translate("Chưa thiết lập thông tin phép, Vui lòng kiểm tra lại."), Utilities.NotifyType.Warning)
                            Exit Sub
                        End If

                        If Date.Now.Day >= dt(0)("WORKDATE_FROM1") AndAlso Date.Now.Day <= dt(0)("WORKDATE_TO1") Then
                            If CommonConfig.AT_ADVANCELEAVETEMP_VALUE() - Date.Now.Month - dt(0)("ANNUAL_WORK1") - objHoliday.ADVANCELEAVE < 0 Then
                                ShowMessage(Translate("Tổng số phép tạm ứng đã vượt quá số phép cho phép trong năm."), Utilities.NotifyType.Warning)
                                Exit Sub

                            Else
                                If CommonConfig.AT_ADVANCELEAVETEMP_VALUE() < (dt1(0)("ADVANCELEAVE") + objHoliday.ADVANCELEAVE) Then
                                    ShowMessage(Translate("Số phép tạm ứng vượt quá quy định cho ứng phép."), Utilities.NotifyType.Warning)
                                    Exit Sub
                                End If
                            End If
                        Else
                            If CommonConfig.AT_ADVANCELEAVETEMP_VALUE() - Date.Now.Month - dt(0)("ANNUAL_WORK2") - objHoliday.ADVANCELEAVE < 0 Then
                                ShowMessage(Translate("Tổng số phép tạm ứng đã vượt quá số phép cho phép trong năm."), Utilities.NotifyType.Warning)
                                Exit Sub
                            Else
                                If CommonConfig.AT_ADVANCELEAVETEMP_VALUE() < (dt1(0)("ADVANCELEAVE") + objHoliday.ADVANCELEAVE) Then
                        ...
                        Select Case CurrentState
                            Case CommonMessage.STATE_NEW
                                If rep.CheckAtAdvanceLeaveEmp_DATE(objHoliday) = False Then
                                    ShowMessage(Translate("Đã tồn tại thông tin đã thiết lập."), Utilities.NotifyType.Warning)
                                    Exit Sub
                                End If
                                If rep.InsertAtAdvanceLeaveEmp(objHoliday, gID) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
                                    IDSelect = gID
                                    Refresh("InsertView")
                                    UpdateControlState()
                            ...
                            Case CommonMessage.STATE_EDIT
                                ...
                                If rep.CheckAtAdvanceLeaveEmp_DATE(objHoliday) = False Then
                                    ...
                                    Exit Sub
                                End If
                                If rep.ModifyAtAdvanceLeaveEmp(objHoliday, rgDanhMuc.SelectedValue) Then
                                    CurrentState = CommonMessage.STATE_NORMAL
    ...
    Protected Sub rgDanhMuc_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles rgDanhMuc.SelectedIndexChanged
        ...
        rdEffectDate.SelectedDate = At_ListParamSystem1.FROMDATE_EFFECT
