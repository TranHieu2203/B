...
Public Class ctrlUserFunctionExceptionAddEdit
    Inherits CommonView
...
#Region "Page"
    ...
    Public Overrides Sub Refresh(Optional ByVal Message As String = "")
        ...
        Try
            ...
            filter.ORG_ID = CDec(Val(hidOrg.Value))
            filter.WORK_PLACE_ID = CDec(Val(cboLocation.SelectedValue))
            ...
            Dim rep As New CommonRepository
            If Sorts IsNot Nothing Then
                UserFunction = rep.GetUserFunctionNotPermisionException(UserID, filter, rgGrid.CurrentPageIndex, PageSize, MaximumRows, Sorts)
            Else
                UserFunction = rep.GetUserFunctionNotPermisionException(UserID, filter, rgGrid.CurrentPageIndex, PageSize, MaximumRows)
            End If
    ...
    Public Overrides Sub BindData()
        ...
        Try
            ...
            If ComboBoxData Is Nothing Then
                ...
                ComboBoxData.GET_JOB_BAND = True
            ...
            FillDropDownList(cboLocation, ComboBoxData.LIST_JOB_BAND, "NAME_VN", "ID", Common.SystemLanguage, True)
            _myLog.WriteLog(_myLog._info, _classPath, method,
                                    CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
            _myLog.WriteLog(_myLog._error, _classPath, method, 0, ex, "")
...
#Region "Event"
        ...
        Try
            ...
            Select Case CType(e.Item, Telerik.Web.UI.RadToolBarButton).CommandName
                Case CommonMessage.TOOLBARITEM_SAVE
                    ...
                    '    ShowMessage("Bạn chưa chọn phòng ban", NotifyType.Warning)
                    '    Exit Sub
                    'End If
                    If cboLocation.CheckedItems.Count = 0 Then
                        ShowMessage("Bạn chưa chọn cấp bậc", NotifyType.Warning)
                        Exit Sub
                    End If
                    If rgGrid.SelectedItems.Count = 0 Then
                        ShowMessage("Bạn chưa chọn bản ghi nào", NotifyType.Warning)
                        Exit Sub
                    End If
                    Dim lst As List(Of CommonBusiness.UserFunctionDTO)
                        ...
                        lst = RepareData()
                    If rep.InsertUserFunctionException(lst, UserInfo.LST_ORG) Then
    ...
    End Sub
    Public Sub check_all(ByVal status As Boolean)
        cbALLOW_CREATE.Checked = status
        cbALLOW_DELETE.Checked = status
        cbALLOW_EXPORT.Checked = status
        cbALLOW_IMPORT.Checked = status
        cbALLOW_MODIFY.Checked = status
        cbALLOW_PRINT.Checked = status
        cbALLOW_SPECIAL1.Checked = status
        cbALLOW_SPECIAL2.Checked = status
        cbALLOW_SPECIAL3.Checked = status
        cbALLOW_SPECIAL4.Checked = status
        cbALLOW_SPECIAL5.Checked = status
    End Sub
    ...
    Protected Function RepareData() As List(Of CommonBusiness.UserFunctionDTO)
        ...
        Try
            ...
            For Each dr As Decimal In SelectedItem
                ...
                item.FUNCTION_ID = dr
                item.USER_ID = UserID
                item.WORK_PLACE_ID = CDec(Val(cboLocation.SelectedValue))
                Dim collection As IList(Of RadComboBoxItem) = cboLocation.CheckedItems
                If (collection.Count <> 0) Then
                    item.LST_WL = New List(Of Decimal)
                    For Each item2 As RadComboBoxItem In collection
                        item.LST_WL.Add(item2.Value)
                    Next
                End If
                lst.Add(item)
            Next
...
#End Region
    ...
    Private Sub ctrlFindOrgPopup_OrganizationSelected(sender As Object, e As OrganizationSelectedEventArgs) Handles ctrlFindOrgPopup.OrganizationSelected
        Dim method As String = System.Reflection.MethodBase.GetCurrentMethod().Name.ToString()
        Dim startTime As DateTime = DateTime.UtcNow
        Try
    ...
    Private Sub cboLocation_ItemCreated(sender As Object, e As RadComboBoxItemEventArgs) Handles cboLocation.ItemCreated
        Refresh()
