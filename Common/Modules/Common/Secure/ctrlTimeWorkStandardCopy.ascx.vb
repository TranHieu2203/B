...
Public Class ctrlTimeWorkStandardCopy
    Inherits CommonView
    ...
    Delegate Sub CopySelectedDelegate(ByVal sender As Object, ByVal e As EventArgs)
    Event CopySelected As CopySelectedDelegate
...
#Region "Property"
    ...
    '' <returns></returns>
    '' <remarks></remarks>
    Public Property LSTIDOLD As List(Of Decimal)
        Get
            Return ViewState(Me.ID & "_LSTIDOLD")
        End Get
        Set(ByVal value As List(Of Decimal))
            ViewState(Me.ID & "_LSTIDOLD") = value
        End Set
    End Property
    ...
    Public ReadOnly Property SelectedID As Decimal?
        Get
            If cboYear.SelectedValue <> "" Then
                Return Decimal.Parse(cboYear.SelectedValue)
...
#Region "Page"
    ...
    Public Overrides Sub BindData()
        ...
        Try
            ...
            table.Rows.Add(row2)
            For index = Date.Now.Year To Date.Now.Year + 2
                row = table.NewRow
                row("ID") = index
                row("YEAR") = index
                table.Rows.Add(row)
            Next
            FillRadCombobox(cboYear, table, "YEAR", "ID", False)
...
#Region "Event"
    ...
    Private Sub btnYES_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnYES.Click
        ...
        Try
            ...
            RaiseEvent CopySelected(sender, e)
            Hide()
            _myLog.WriteLog(_myLog._info, _classPath, method,
                                    CLng(DateTime.UtcNow.Subtract(startTime).TotalSeconds).ToString(), Nothing, "")
        Catch ex As Exception
