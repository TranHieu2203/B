...
namespace HiStaffAPI.AppCommon.DAO
{
    ...
    {
        public readonly string ConnectionString = ConfigurationManager.AppSettings["DbConnection"];
        ...
        public OracleDBManager(string commandText, CommandType commandType, string connectionString)
        {
            _command = new OracleCommand(commandText);
            _command.CommandTimeout = 120;
            _command.CommandType = commandType;
            ...
            {                                                                                                           //OpenDB(ref Conn);
                using (OracleConnection Conn = new OracleConnection(ConnectionString))
                {
                                        ...
                                        for (int i = 0; i < reader.FieldCount; i++)
                                        {
                                            try
                                            {
                                                ...
                                                if (",char,nchar,nclob,nvarchar2,clob,varchar2,".IndexOf("," + dataType.ToLower() + ",") > -1)
                                                    jsonSub = jsonSub + "," + "\"" + reader?.GetName(i)?.ToUpper() + "\":\"" + (reader?.GetValue(i) is null ? "" : reader?.GetValue(i)?.ToString()) + "\"";
                                                ...
                                                else if (",date,timestamp,timestampltz,timestamptz,".IndexOf("," + dataType?.ToLower() + ",") > -1)
                                                {
                                                    ...
                                                    {
                                                        DateTime outDate = (DateTime)(reader.GetValue(i));
                                                        String outString = outDate.Year.ToString() +"/"+ outDate.Month.ToString("00") +"/"+ outDate.Day.ToString("00") 
                                                          +" "  + outDate.Hour.ToString("00")+":"+ outDate.Minute.ToString("00") + ":"+ outDate.Second.ToString("00");
                                                        jsonSub = jsonSub + "," + "\"" + reader.GetName(i).ToUpper() + "\":\"" + outString + "\"";
                                                    }
                                                }else if (",number,".IndexOf("," + dataType?.ToLower() + ",") > -1 )
                                                {
                                            ...
                                            {
                                                jsonSub = jsonSub + "," + "\"" + reader?.GetName(i)?.ToUpper() + "\":\"\"";
                                                LogHelper.WriteExceptionToLog(String.Format("errorString: {0}\njson: {1}\nex: {2}", errorString, json, ex.Message ));
                                                continue;
                                            }
                                        }
                    ...
                    catch (Exception e)
                    {
                        ...
                        System.Diagnostics.StackFrame f = st.GetFrame(0);
                        json = "{\"" + functionName + "\": {\"Count\": " + RSCount + ", \"Items\":[]}}"; RSCount = 0; errorCode = 500;
                        LogHelper.WriteExceptionToLog(String.Format("errorString: {0}\njson: {1}\ne: {2}", errorString, json, e.Message+f.GetFileLineNumber().ToString ()));
                    }
                    //CloseDB(ref Conn);
                    //RS = null;
                }
                ...
                System.Diagnostics.StackFrame f = st.GetFrame(0);
                errorString =ex.ToString()+ f.GetFileLineNumber().ToString();
            }
        }
