...
namespace HiStaffAPI.AppCommon.Authen
{
    public class Authen : IAuthen
    {
        ...
        private void GetHistaffUserByEmail(string Username, string DeviceID, ref string json, ref int errorCode, ref string errorString)// API Mobile
        {
            ...
            using (var conn = new OracleDBManager("SP_CMS__Users_GetInfoByEmail", System.Data.CommandType.StoredProcedure, ConnectionString))
            {
                conn.ExecuteStoreAPI("login", "SP_CMS__Users_GetInfoByEmail", d1, ref parameterOutput, ref json, ref errorCode, ref errorString);

            }
            //_bosDao.ExecuteStoreAPI("login", "SP_CMS__Users_GetInfoByEmail", d1, ref parameterOutput, ref json, ref errorCode, ref errorString);
        }
        ...
        private void GetHistaffUserByEmail(string Username, out dynamic d)// Web app
        {
            ...
            //_bosDao.ExecuteStore("Authen", "SP_CMS__Users_GetInfoByEmail", d1, ref parameterOutput, ref json, ref errorCode, ref errorString, false);
            ...
            out string domain, out string smtp, out int port, out bool IsSSL)// Mail server
        {
            try
            {
                ...
                using (var conn = new OracleDBManager("SP_CMS__Sys_MailServer_List", System.Data.CommandType.StoredProcedure, ConnectionString))
                {
                ...
                //_companyDao.ExecuteStoreToDataset("MailServer", "SP_CMS__Sys_MailServer_List", d1, ref parameterOutput, ref json, ref errorCode, ref errorString, true);
            ...
            out string LdapURL, out string LDAPDomain, out string LDAPBaseDN)// Ldap
        {
            try
            {
                ...
                using (var conn = new OracleDBManager("SP_CMS__Sys_LDAP_List", System.Data.CommandType.StoredProcedure, ConnectionString))
                {
                ...
                //_companyDao.ExecuteStoreToDataset("LdapServer", "SP_CMS__Sys_LDAP_List", d1, ref parameterOutput, ref json, ref errorCode, ref errorString, true);
                d1 = JObject.Parse(json);
        ...
        public void LoginWithHistaffUser(LoginRequest request, ref string json, ref int errorCode, ref string errorString) // API Mobile
        {
            ...
            dynamic StoreParam = JObject.Parse("{\"parameterInput\":[" +
                ...
                "{\"ParamName\": \"Avatar\", \"ParamType\": \"12\", \"ParamInOut\": \"3\", \"ParamLength\": \"400\", \"InputValue\": \"\"}," +
                "{\"ParamName\": \"Token\", \"ParamType\": \"12\", \"ParamInOut\": \"3\", \"ParamLength\": \"200\", \"InputValue\": \"\"}," +
                "{\"ParamName\": \"IsAd\", \"ParamType\": \"12\", \"ParamInOut\": \"3\", \"ParamLength\": \"200\", \"InputValue\": \"\"}," +
                "{\"ParamName\": \"LdapName\", \"ParamType\": \"12\", \"ParamInOut\": \"3\", \"ParamLength\": \"600\", \"InputValue\": \"\"}," +
                "{\"ParamName\": \"LdapBase\", \"ParamType\": \"12\", \"ParamInOut\": \"3\", \"ParamLength\": \"600\", \"InputValue\": \"\"}," +
                "{\"ParamName\": \"DomainName\", \"ParamType\": \"12\", \"ParamInOut\": \"3\", \"ParamLength\": \"600\", \"InputValue\": \"\"}," +
                "{\"ParamName\": \"Message\", \"ParamType\": \"12\", \"ParamInOut\": \"3\", \"ParamLength\": \"600\", \"InputValue\": \"\"}," +
                "{\"ParamName\": \"ResponseStatus\", \"ParamType\": \"8\", \"ParamInOut\": \"3\", \"ParamLength\": \"4\", \"InputValue\": \"0\"}]}");

            using (var conn = new OracleDBManager(ProcedureName.API_PackageDefault + "." + ProcedureName.API_User_Login, CommandType.StoredProcedure, ConnectionString))
            {
                conn.ExecuteStoreAPI("login", ProcedureName.API_PackageDefault + "." + ProcedureName.API_User_Login, StoreParam, ref parameterOutput, ref json, ref errorCode, ref errorString);
            }
            ...
            try
            {
                ...
                var IsAd = (long.Parse(dJson["login"].ParameterOutput.IsAd.ToString()));
                if (IsAd == -1)
                {
                    //check validate ldap...
                    var LdapURL = ""; var LDAPDomain = ""; var LDAPBaseDN = "";
                    LdapURL = dJson["login"].ParameterOutput.LdapName.Value;
                    LDAPDomain = dJson["login"].ParameterOutput.DomainName.Value;
                    LDAPBaseDN = dJson["login"].ParameterOutput.LdapBase.Value;
                    Boolean bSuccess = ADManager.IsUserExists(LdapURL.ToString(), request.UserName, request.Password, LDAPBaseDN);
                    //LDAP m = new LDAP(LdapURL.ToString(), LDAPDomain.ToString(), LDAPBaseDN.ToString(), 0, request.UserName, request.Password); // 0 - ContextType.Machine
                    if (!bSuccess)
                    {
                        mess = request.Language == "en-US" ? "LDAP sign in failed" : "Đăng nhập tài khoản LDAP không thành công";
                        json = "{\"login\":{\"ParameterOutput\": {\"CompanyCode\":\" \",\"UserID\": 0,\"FullName\":\"\",\"Email\":\"\",\"Mobile\":\"\",\"Avatar\":\"\",\"Token\":\"\",\"IsAd\":\"-1\",\"LdapName\":\"\",\"LdapBase\":\"\",\"DomainName\":\"\", \"Message\": \"" + mess + "\", \"ResponseStatus\": -99}, \"ResponseStatus\": -99, \"Message\": \"" + mess + "\",  \"Count\": 0, \"Items\":[]}}";
            ...
            {
                mess = request.Language == "en-US" ? "Incorect Username or password" : "Tài khoản hoặc mật khẩu không đúng";
                json = "{\"login\":{\"ParameterOutput\": {\"CompanyCode\":\" \",\"UserID\": 0,\"FullName\":\"\",\"Email\":\"\",\"Mobile\":\"\",\"Avatar\":\"\",\"Token\":\"\",\"IsAd\":\"-1\",\"LdapName\":\"\",\"LdapBase\":\"\",\"DomainName\":\"\", \"Message\": \"" + mess + "\", \"ResponseStatus\": -99}, \"ResponseStatus\": -99, \"Message\": \"" + mess + "\", \"Count\": 0, \"Items\":[]}}";
            }
        ...
        public ExecuteResult CheckToken(TokenApiDTO _token, string functionName)
        {
            ...
            return ExecuteStore(request, ProcedureName.API_PackageDefault + "." + ProcedureName.API_User_LoginWToken, functionName, JObject.FromObject(StoreParam));
        }
        ...
        public async Task<ExecuteResult> CheckPermission(string userID, string functionName)
        {
            return await Task.Run(() =>
            {
                ...
                using (var conn = new OracleDBManager(ProcedureName.API_PackageDefault + "." + ProcedureName.API_User_CheckPermission, System.Data.CommandType.StoredProcedure, ConnectionString))
                {
                    conn.ExecuteStoreAPI(functionName, ProcedureName.API_PackageDefault + "." + ProcedureName.API_User_CheckPermission, d, ref parameterOutput, ref json, ref errorCode, ref errorString);
        ...
        public void LoginWithHistaffUser(string Username, string UserPassword, out dynamic d) //Web app
        {
            ...
            using (var conn = new OracleDBManager(ProcedureName.API_PackageDefault + "." + ProcedureName.API_User_Login, System.Data.CommandType.StoredProcedure, ConnectionString))
            {
                conn.ExecuteStoreAPI("Authen", ProcedureName.API_PackageDefault + "." + ProcedureName.API_User_Login, d1, ref parameterOutput, ref json, ref errorCode, ref errorString, false);
            }
            //_bosDao.ExecuteStore("Authen", "SP_CMS__Users_Login", d1, ref parameterOutput, ref json, ref errorCode, ref errorString, false);
            d = JObject.Parse("{" + parameterOutput + "}");
        }
            ...
            if (getUser.PASSWORD == UserPassword)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var token = TokenHelper.GenerateToken("HiStaffAPI", getUser.USERNAME, getUser.EMPLOYEE_ID, getUser.EMPLOYEE_CODE, "login", "", true, DeviceID);//Generate token
