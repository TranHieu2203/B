...
Public Class ctrlPortalResultRecord
...
#Region "Event"
    ...
    Protected Sub rnDIRECT_POINT_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        Try
            Dim edit = CType(sender, RadNumericTextBox)
            Dim row = CType(edit.NamingContainer, GridEditableItem)
            Dim directPoint = edit.Value
            Dim id = row.GetDataKeyValue("ID")
            If Not IsNumeric(directPoint) AndAlso directPoint <> "" Then
        ...
        End Try
    End Sub
    Protected Sub txtDIRECT_ACTUAL_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        Try
            Dim edit = CType(sender, RadTextBox)
            Dim row = CType(edit.NamingContainer, GridEditableItem)
            Dim directActual = edit.Text
            Dim id = row.GetDataKeyValue("ID")
            Dim code = If(row.GetDataKeyValue("TARGET_TYPE_CODE") Is Nothing, "", row.GetDataKeyValue("TARGET_TYPE_CODE").ToString)
            If code = "NUMBER" Then
                If Not IsNumeric(directActual) AndAlso directActual <> "" Then
                    ShowMessage(Translate("Chỉ được nhập số"), NotifyType.Warning)
                    RebindGrid()
                    Exit Sub
                Else
                    For Each item As PE_KPI_ASSESMENT_DETAIL_DTO In dtDetail
                        If item.ID = id Then
                            item.DIRECT_ACTUAL = directActual
            ...
            ElseIf code = "DATE" Then
                If Not IsDate(directActual) AndAlso directActual <> "" Then
...
#Region "Custom"
    ...
    Private Sub SetDataToGrid(ByVal EditItem As GridEditableItem)
        Dim txtDIRECT_ACTUAL, txtNOTE_QLTT As New RadTextBox
        Dim rnDIRECT_POINT As New RadNumericTextBox
        Try
            txtDIRECT_ACTUAL = CType(EditItem.FindControl("txtDIRECT_ACTUAL"), RadTextBox)
            rnDIRECT_POINT = CType(EditItem.FindControl("rnDIRECT_POINT"), RadNumericTextBox)
            txtNOTE_QLTT = CType(EditItem.FindControl("txtNOTE_QLTT"), RadTextBox)
            Dim id = EditItem.GetDataKeyValue("ID")
            For Each item As PE_KPI_ASSESMENT_DETAIL_DTO In dtDetail
                If item.ID = id Then
                    txtDIRECT_ACTUAL.Text = item.DIRECT_ACTUAL
                    rnDIRECT_POINT.Value = item.DIRECT_POINT
                    txtNOTE_QLTT.Text = item.NOTE_QLTT
                End If
            Next
        Catch ex As Exception
            DisplayException(Me.ViewName, Me.ID, ex)
