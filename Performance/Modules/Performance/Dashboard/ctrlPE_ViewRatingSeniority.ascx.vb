...
Public Class ctrlPE_ViewRatingSeniority
...
#Region "Page"
    ...
    Public Overrides Sub BindData()
        ...
        End Try
    End Sub
    Private Function Change(ByVal text As String) As String
        If text = "MOT" Then
            Return "1 năm"
        End If
        If text = "HAI" Then
    ...
    Public Overrides Sub Refresh(Optional ByVal Message As String = "")
        ...
        Try
            ...
            If StatisticData Is Nothing Or Message = CommonMessage.ACTION_UPDATED Then
                StatisticData = rep.GetStatisticKPISeniority(Utilities.ObjToInt(cboPeriod.SelectedValue))
            End If
            If StatisticData IsNot Nothing And StatisticData.Rows.Count > 0 Then
                For index As Integer = 0 To StatisticData.Columns.Count - 1
                    If StatisticData.Columns(index).ColumnName <> "NAME" And StatisticData.Columns(index).ColumnName <> "VALUE_SUM" Then
                        If index = StatisticData.Columns.Count - 1 Then
                            categories &= "'" & Change(StatisticData.Columns(index).ColumnName) & "'"
                        Else
                            categories &= "'" & Change(StatisticData.Columns(index).ColumnName) & "',"
                        End If
                    End If
                Next
                For Each item As DataRow In StatisticData.Rows
                    data &= "{name: '" & Translate(item("NAME")) & "',data: ["
                    For index As Integer = 0 To StatisticData.Columns.Count - 1
                        If StatisticData.Columns(index).ColumnName <> "NAME" And StatisticData.Columns(index).ColumnName <> "VALUE_SUM" Then
                            If index = StatisticData.Columns.Count - 1 Then
                                data &= item(StatisticData.Columns(index).ColumnName)
                            Else
                                data &= item(StatisticData.Columns(index).ColumnName) & ","
                            End If
                        End If
