...
Public Class ctrlPE_EmployeePeriod
            ...
            Select Case CType(e.Item, RadToolBarButton).CommandName
                ...
                Case CommonMessage.TOOLBARTIEM_CALCULATE
                    ...
                    If cboPeriod.SelectedValue = "" Then
                        ...
                        Exit Sub
                    End If
                    If rep.CAL_EMPLOYEE_PERIOD(_org_id, cboPeriod.SelectedValue) Then
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_SUCCESS), NotifyType.Success)
                        rgData.Rebind()
                    Else
                        ShowMessage(Translate(CommonMessage.MESSAGE_TRANSACTION_FAIL), NotifyType.Error)
                ...
                Case CommonMessage.TOOLBARITEM_SENDMAIL
                    ...
                    If Not cboPeriod.SelectedValue <> "" Then
                    ...
                    For Each itemSelect As GridDataItem In rgData.SelectedItems
                        Dim mail As String = If(itemSelect.GetDataKeyValue("EMAIL") Is Nothing, String.Empty, itemSelect.GetDataKeyValue("EMAIL").ToString)
                        If String.IsNullOrEmpty(mail) Then
                            ShowMessage("Tồn tại nhân viên chưa có email", NotifyType.Warning)
                            Exit Sub
                        End If
                    Next

                    dataMail = store.GET_MAIL_TEMPLATE("VP_THONGBAO_DG", "Performance")
                    ...
                    For Each itemSelect As GridDataItem In rgData.SelectedItems
                        ...
                        Dim mail As String = If(itemSelect.GetDataKeyValue("EMAIL") Is Nothing, String.Empty, itemSelect.GetDataKeyValue("EMAIL").ToString)
                        Dim send_mail As Boolean = itemSelect.GetDataKeyValue("SEND_MAIL")
                        If (Not String.IsNullOrEmpty(mail)) And (Not send_mail) Then
                            Dim body As String = dataMail.Rows(0)("CONTENT").ToString
                            dtValues = store.GET_MAIL_INFORMATION_OF_PE_EMPLOYEE_PERIOD(employee_id, cboPeriod.SelectedValue)
                            ...
                            body = Replace(body, "SO_NGAY", dtValues.Rows(0)("SO_NGAY").ToString())
                            body = Replace(body, "SO_THANG", dtValues.Rows(0)("SO_THANG").ToString())
                            If psp.InsertMail(defaultFrom, mail, titleMail, body, "", "", "VP_THONGBAO_DG", True) Then
        ...
        Try
            ...
            If isFull Then
                If Sorts IsNot Nothing Then
                    Return rep.GetEmployeePeriods(_filter, 0, Integer.MaxValue, 0, lstOrg, Sorts).ToTable
                Else
                    Return rep.GetEmployeePeriods(_filter, 0, Integer.MaxValue, 0, lstOrg).ToTable
                End If
            Else
                Dim EmpList As New List(Of EmployeePeriodDTO)
                If Sorts IsNot Nothing Then
                    EmpList = rep.GetEmployeePeriods(_filter, rgData.CurrentPageIndex, rgData.PageSize, MaximumRows, lstOrg, Sorts)
                Else
                    EmpList = rep.GetEmployeePeriods(_filter, rgData.CurrentPageIndex, rgData.PageSize, MaximumRows, lstOrg)
                End If
