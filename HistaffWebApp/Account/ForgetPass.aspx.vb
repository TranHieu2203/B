...
#Region "Page"
    Public Overrides Sub PageLoad(ByVal e As System.EventArgs)
        ...
        If Not IsPostBack Then
            ...
            countCode = 0
        End If
        hidEffectTimeForCodeResetPassword.Value = CommonConfig.EffectTimeForCodeResetPassword
    End Sub
...
#Region "Event"
        ...
        Try
            If Page.IsValid Then
                ...
                ElseIf result = "3" Then
                    ShowMessage(Translate("Tài khoản đã bị khóa/User Is Locked"), Utilities.NotifyType.Warning)
                    Exit Sub
                End If
                ...
                body = dataMail(0)
                titleMail = dataMail(2) ' "Forget password"
                ...
                bodyNew = String.Format(body, ChangeCode, CommonConfig.EffectTimeForCodeResetPassword)
                ...
                lstID.Add(uID)
                If Not rep.SendMailConfirmUserPassword(lstID, titleMail, bodyNew, 1) Then
                    ShowMessage(Translate("Đã xảy ra lỗi"), NotifyType.Warning)
                    Exit Sub
                Else
                    ShowMessage(Translate("Hệ thống đã gửi mã xác nhận về email của bạn. Lưu ý mã xác nhận chỉ tồn tại trong " & CommonConfig.EffectTimeForCodeResetPassword & "s!"), NotifyType.Warning)
                    Session("START_CODE") = DateTime.Now
                    Panel1.Visible = False
                    Panel2.Visible = True
                    ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType, "", "startTimer();", True)
                End If
            End If
    ...
    Private Sub btnSubmitCode_Click(sender As Object, e As System.EventArgs) Handles btnSubmitCode.Click
        ...
        Try
            If Page.IsValid Then
                If DateDiff(DateInterval.Second, CDate(Session("START_CODE")), DateTime.Now) > CommonConfig.EffectTimeForCodeResetPassword Then
                ...
                Panel3.Visible = True
                ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType, "", "clearInterval(timeDown);", True)
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub btnCancel_Click(sender As Object, e As System.EventArgs) Handles btnCancel.Click, btnCodeCancel.Click, btnPassCancel.Click, btnResendCodeCancel.Click
        Dim str As String = "getRadWindow().Close('0');"
        Session.Remove("START_CODE")
        ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType, "clientButtonClicking", str, True)
    End Sub

    Private Sub btnResendCode_Click(sender As Object, e As System.EventArgs) Handles btnResendCode.Click
        Dim rep As New CommonRepository
        Dim dataMail As List(Of String)
        Dim body As String = ""
        Dim chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
        ...
        Try
            ...
            'If Not Common.Common.sendEmailByServerMail(result,
            '                                                If(dataMail(1) IsNot Nothing, dataMail(1), result),
            '                                                titleMail, bodyNew, String.Empty) ThenDim lstID As New List(Of Decimal)
            Dim lstID As New List(Of Decimal)
            lstID.Add(uID)
            If Not rep.SendMailConfirmUserPassword(lstID, titleMail, bodyNew, 1) Then
                ShowMessage(Translate("Đã xảy ra lỗi"), NotifyType.Warning)
    ...
    Private Sub btnShowResendCode_Click(sender As Object, e As System.EventArgs) Handles btnShowResendCode.Click
        Panel2.Visible = False
        Panel4.Visible = True
    End Sub
#End Region
