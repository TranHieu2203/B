<%@ Control Language="vb" AutoEventWireup="false" CodeBehind="ctrlRC_Request_Person_TCTD.ascx.vb"
    Inherits="Recruitment.ctrlRC_Request_Person_TCTD" %>
    ...
    <tlk:RadPane ID="MainPane" runat="server" Scrolling="None">
        <tlk:RadSplitter ID="RadSplitter3" runat="server" Width="100%" Height="100%" Orientation="Horizontal" Scrolling="None">
            ...
            <tlk:RadPane ID="RadPane2" runat="server" Scrolling="None">
                <tlk:RadGrid ID="rgData" runat="server" Height="100%" AllowMultiRowSelection="True" SkinID="GridSingleSelect">
                    <ClientSettings EnableRowHoverStyle="true">
                        <Selecting AllowRowSelect="true" />
                    </ClientSettings>
                    <MasterTableView DataKeyNames="ID,IS_CLOCK" ClientDataKeyNames="ID,CODE_RC,NAME_RC,REQUIRER_NAME,STATUS_ID,RC_RECRUIT_PROPERTY,RC_RECRUIT_PROPERTY_NAME,IS_OVER_LIMIT,FOREIGN_ABILITY,COMPUTER_APP_LEVEL,GENDER_PRIORITY,GENDER_PRIORITY_NAME,RECRUIT_NUMBER,UPLOAD_FILE,IS_CLOCK">
                        <Columns>
                           ...
                           <tlk:GridBoundColumn HeaderText="<%$ Translate: Mã YCTD %>" DataField="CODE_RC"
                            ...
                            <tlk:GridBoundColumn HeaderText="<%$ Translate: Trạng thái %>" DataField="STATUS_NAME"
                                SortExpression="STATUS_NAME" UniqueName="STATUS_NAME" Visible="false"/>
                            <tlk:GridBoundColumn HeaderText="ORG_DESC" DataField="ORG_DESC" UniqueName="ORG_DESC"
                                SortExpression="ORG_DESC" Visible="false" />
                            <tlk:GridBoundColumn HeaderText="<%$ Translate: IS LOCK %>" DataField="IS_CLOCK"
                                SortExpression="IS_CLOCK" UniqueName="IS_CLOCK" Visible="false"/>
                            <tlk:GridBoundColumn HeaderText="<%$ Translate: Trạng thái %>" DataField="IS_CLOCK_NAME"
                                SortExpression="IS_CLOCK_NAME" UniqueName="IS_CLOCK_NAME" Visible="true"/>
                        </Columns>
                    </MasterTableView>
                    <ClientSettings>
                        <Selecting AllowRowSelect="True" />
...
<tlk:RadCodeBlock ID="RadCodeBlock1" runat="server">
    <script type="text/javascript">
        ...
        function OpenEdit() {
            ...
            var islock = $find('<%# rgData.ClientID%>').get_masterTableView().get_selectedItems()[0].getDataKeyValue('IS_CLOCK');
            window.open('/Default.aspx?mid=Recruitment&fid=ctrlRC_RequestNewEdit_TBP&group=Business&noscroll=1&ID=' + id +'&IS_CLOCK=' + islock, "_self");
