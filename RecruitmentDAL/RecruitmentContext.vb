...
Partial Public Class RecruitmentContext
    Public Overloads Function SaveChanges(ByVal log As UserLog) As Integer
        ...
        For Each entry As ObjectStateEntry In entries
            ...
            Using enHelp As New EntityHelper
                If Auditable IsNot Nothing Then
                    ...
                    Else
                        ObjectId = enHelp.GetProperty(Auditable, "ID")
                    End If

                    If entry.State = EntityState.Added Then
                        enHelp.SetProperty("CREATED_BY", log.Username, Auditable)
                        enHelp.SetProperty("CREATED_DATE", DateTime.Now, Auditable)
                        enHelp.SetProperty("CREATED_LOG", log.Ip & "-" & log.ComputerName, Auditable)
                        enHelp.SetProperty("MODIFIED_BY", log.Username, Auditable)
                        enHelp.SetProperty("MODIFIED_DATE", DateTime.Now, Auditable)
                        enHelp.SetProperty("MODIFIED_LOG", log.Ip & "-" & log.ComputerName, Auditable)
                        ObjectState = "I"
                        IsAudit = True
                    ...
                    If IsAudit AndAlso AuditLogHelper.IsAuditLog(ObjectName) Then
                        Dim lstLogDtl = enHelp.GetEntryValueInString(entry)
